---
id: wiki:dependencies
title: Dependency Map
sources:
  - llm-wiki/graph.json
generatedFromGraphHash: 67fdf30e687528e70baefc61cc604eb1b059c261dba1a5780da081c3af5a82bb
generatedAt: 2026-05-26T16:35:12.352Z
---

# Dependency Map

Dependencies stay queryable in the graph while this page gives the human overview.

## Summary

- Resolved local import edges: 1200
- Package import edges: 585
- Unresolved local import edges: 2

## Package Imports

| Package | Importing Files |
| --- | ---: |
| `node:path` | 72 |
| `@pencil-agent/tui` | 60 |
| `@pencil-agent/ai` | 50 |
| `node:fs` | 50 |
| `@pencil-agent/agent-core` | 43 |
| `fs` | 30 |
| `path` | 28 |
| `node:os` | 26 |
| `@sinclair/typebox` | 25 |
| `node:fs/promises` | 22 |
| `node:url` | 17 |
| `node:crypto` | 15 |
| `child_process` | 14 |
| `node:child_process` | 14 |
| `chalk` | 9 |
| `os` | 8 |
| `node:http` | 6 |
| `fs/promises` | 5 |
| `node:module` | 5 |
| `node:util` | 5 |
| `readline` | 5 |
| `@google/genai` | 4 |
| `node:events` | 4 |
| `openai` | 4 |
| `openai/resources/responses/responses.js` | 4 |
| `url` | 4 |
| `proper-lockfile` | 3 |
| `strip-ansi` | 3 |
| `vitest/config` | 3 |
| `yaml` | 3 |
| `ajv` | 2 |
| `diff` | 2 |
| `ignore` | 2 |
| `marked` | 2 |
| `minimatch` | 2 |
| `module` | 2 |
| `node:https` | 2 |
| `node:process` | 2 |
| `node:readline` | 2 |
| `node:worker_threads` | 2 |
| `@agentclientprotocol/sdk` | 1 |
| `@anthropic-ai/sdk` | 1 |
| `@anthropic-ai/sdk/resources/messages.js` | 1 |
| `@aws-sdk/client-bedrock-runtime` | 1 |
| `@mariozechner/jiti` | 1 |
| `@pencil-agent/nano-pencil` | 1 |
| `@pencil-agent/soul` | 1 |
| `@pencil-agent/soul-core` | 1 |
| `@silvia-odwyer/photon-node` | 1 |
| `@sinclair/typebox/compiler` | 1 |
| `ajv-formats` | 1 |
| `crypto` | 1 |
| `events` | 1 |
| `extract-zip` | 1 |
| `file-type` | 1 |
| `get-east-asian-width` | 1 |
| `glob` | 1 |
| `hosted-git-info` | 1 |
| `node:stream` | 1 |
| `openai/resources/chat/completions.js` | 1 |
| `partial-json` | 1 |
| `stream` | 1 |
| `stream/promises` | 1 |
| `typescript` | 1 |
