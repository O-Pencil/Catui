/**
 * [WHO]: Verifies CATAIL default registration, resource discovery, bootstrap behavior, and deterministic artifact audit
...
function createApiHarness(): { api: ExtensionAPI; handlers: Map<string, Handler[]> } {
...
test("CATAIL discovers one skill and injects a bounded evidence reminder", async () => {
	const { api, handlers } = createApiHarness();
	...
	assert.match(result.appendSystemPrompt ?? "", /Model output, search snippets, hypotheses, and fluent drafts are not evidence/);
...
function copyResolvedTemplate(name: string, destination: string) {
	...
	writeFileSync(destination, readFileSync(source, "utf8").replaceAll("unresolved", "resolved"), "utf8");
}
...
test("CATAIL audit accepts a structurally complete evidence trail", () => {
	...
	try {
		...
		assert.deepEqual(result.checks, { searches: 1, claims: 1, analyses: 1, evidence: 1 });
	} finally {
