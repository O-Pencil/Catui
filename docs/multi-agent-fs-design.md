...
> **本文档是 nanoPencil + Pencil-Agent-Gateway 多 Agent 改造的唯一权威源**。
> 上一版本（80 行精简稿）的演进 4 阶段保留为 §12 的迁移大纲；
> 完整设计、目录布局、Gateway/nanoPencil 双仓库改造说明全部在此一文。
>
...
| 阶段 | nanoPencil 任务 | Gateway 任务 | 阻塞关系 |
|------|----------------|-------------|---------|
| **Phase 1：基础设施** | N1 `AgentDirContext` 抽象 · N2 `PENCILS_HOME` env 支持 · N3 `team-state-store` env bug 修复 | — | 无阻塞，可立即启动 |
| **Phase 2：硬编码改造** | N4 persona · N5 session · N6 soul · N7 mcp · N8 audit-log | — | 依赖 N1 |
| **Phase 3：用户可见** | N9 `--agent <id>` · N10 `agent.json` · N11 `pencils migrate` · N12 切 `CONFIG_DIR_NAME` | G2 ID 校验正则 · G3 注册时写 `agent.json` | 依赖 N1–N8；G2/G3 与 N10 协调 |
| **Phase 4：Workspace** | N13 `WorkspaceManager` · N14 `ws_id` 派生 · N15 sessions 双轴索引 | G6 `/v1/workspaces` API · `AgentConfig.workspaceId` | 依赖 N9 |
| **Phase 5：Teams** | N16 remote teammate · N17 team-state 落 workspace · N18 mailbox→黑板 · N19 移除 legacy | G7 `/v1/teams/<id>/dispatch` | 依赖 N13 |
...
| **N3** | `team-state-store.ts` env 名修复（`NANOPENCIL_AGENT_DIR` → `NANOPENCIL_CODING_AGENT_DIR`，复用 config.ts 的 `getAgentDir`）—— **independent latent bugfix** | S | — |
...
| **N11** | `pencils migrate` 子命令（保守迁移，dry-run 默认开）| M | N1, N3 |
...
$ pencils migrate --apply              # 实际执行；写 .migrations/applied.jsonl
```
...
| 操作 | 回退 |
|---|---|
| `pencils migrate --apply` | `pencils migrate --rollback` 从 `.backups/<ts>-migrate/` 恢复 |
