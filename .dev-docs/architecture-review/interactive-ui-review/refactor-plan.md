...
| done | `input-submit-controller` | UI06 | optimistic/bashMode/排队决策 | 已落地：submit 总分派移出 mount，保留 slash/image/bash/session/render owner 边界；清理已被 slash-dispatcher 覆盖的 standalone slash 死分支；token 中性 |
...
| done(目标修正) | `interactive-mode.ts` → mount | — | 根容器 | **退壳评估结案（scope C）**：`<500` 作废（不可达且失真——mount = 组合根 + ~12 controller 的 port 面，地板 ~1500-1700）；危险耦合已全部出 mount，残留为扁平低耦合 handler。S-3"无冗余"判据为 god 已拆 + verify-quality PASS，非单文件 <500。resources-display(481)/slash-handlers(981) 等列 **interactive 域内 post-P5 可选清理**（非正式 phase P6=入口体积）。结构封口后正式主线接 phase P6。见 mount-shell-evaluation.md §4/§8 |
| done | `stream-render-controller`（handleEvent render 层）| UI04 | 流式/工具/loader/retry+compaction overlay | 已落地（scope A）：12-case switch 纯搬入 render controller（7 组 port）；retry/compaction loader+escape 4 字段移出 interactive-state 收为私有；mount handleEvent → streamRender.handle；token 中性。解锁 #8 退壳。见 handle-event-analysis.md §6/§9 |
