/**
 * Anti-Pattern Browser Detector for Catpaw
...
function checkElementQualityDOM(el) {
  ...
  const rect = el.getBoundingClientRect();
  const lineMax = (typeof window !== 'undefined' && window.__CATPAW_CONFIG__?.lineLengthMax) || 80;
...
function checkTypography() {
  ...
  for (const el of document.querySelectorAll('p, h1, h2, h3, h4, h5, h6, li, td, th, dd, blockquote, figcaption, a, button, label, span')) {
    // Skip catpaw's own elements
    if (el.closest && el.closest('.catpaw-overlay, .catpaw-label, .catpaw-banner, .catpaw-tooltip')) continue;
...
if (IS_BROWSER) {
  ...
  const EXTENSION_MODE = (_myScript && _myScript.dataset.catpawExtension === 'true')
    || document.documentElement.dataset.catpawExtension === 'true';
  ...
  styleEl.textContent = `
    @keyframes catpaw-reveal {
      from { opacity: 0; }
      to { opacity: 1; }
    }
    .catpaw-overlay:not(.catpaw-banner) {
      pointer-events: none;
      outline: 2px solid ${OUTLINE_COLOR};
      border-radius: 4px;
      transition: outline-color 0.15s ease;
      animation: catpaw-reveal 0.4s cubic-bezier(0.16, 1, 0.3, 1) both;
      animation-play-state: paused;
      border-top-left-radius: 0;
    }
    .catpaw-overlay.catpaw-visible {
      animation-play-state: running;
    }
    .catpaw-overlay.catpaw-hover {
      outline-color: ${BRAND_COLOR_HOVER};
      z-index: 100001 !important;
    }
    .catpaw-overlay.catpaw-hover .catpaw-label {
      background: ${BRAND_COLOR_HOVER};
    }
    .catpaw-overlay.catpaw-spotlight {
      z-index: 100002 !important;
    }
    .catpaw-overlay.catpaw-spotlight-dimmed {
      opacity: 0.15 !important;
      animation: none !important;
      filter: blur(3px);
    }
    .catpaw-spotlight-backdrop {
      position: fixed;
    ...
    .catpaw-spotlight-backdrop.catpaw-visible {
      opacity: 1;
    }
    .catpaw-hidden .catpaw-overlay${EXTENSION_MODE ? '' : ':not(.catpaw-banner)'} {
  ...
  function getSpotlightBackdrop() {
    if (!spotlightBackdrop) {
      ...
      spotlightBackdrop.className = 'catpaw-spotlight-backdrop';
  ...
  function showSpotlight(target) {
    ...
    if (window.__CATPAW_CONFIG__?.spotlightBlur === false) {
    ...
    bd.classList.add('catpaw-visible');
  }

  function hideSpotlight() {
    spotlightTarget = null;
    if (spotlightBackdrop) spotlightBackdrop.classList.remove('catpaw-visible');
  }
  ...
  function repositionOverlays() {
    for (const o of overlays) {
      if (!o._targetEl || o.classList.contains('catpaw-banner')) continue;
  ...
  const visibilityObserver = new IntersectionObserver((entries) => {
    for (const entry of entries) {
      const overlay = entry.target._catpawOverlay;
      if (!overlay) continue;
      if (entry.isIntersecting) {
        ...
        if (!overlay._revealed) {
          ...
          requestAnimationFrame(() => {
            overlay.classList.add('catpaw-visible');
  ...
  function detachOverlay(overlay) {
    ...
    if (overlay._targetEl && overlay._targetEl._catpawOverlay === overlay) {
      visibilityObserver.unobserve(overlay._targetEl);
      delete overlay._targetEl._catpawOverlay;
    }
  ...
  document.addEventListener('transitionend', (e) => {
    ...
    for (const o of overlays) {
      if (!o._targetEl || o.classList.contains('catpaw-banner') || o.style.display === 'none') continue;
  ...
  const highlight = function(el, findings) {
    ...
    const onMouseEnter = () => {
      isHovered = true;
      outline.classList.add('catpaw-hover');
    ...
    const onMouseLeave = () => {
      isHovered = false;
      outline.classList.remove('catpaw-hover');
  ...
  const showPageBanner = function(findings) {
    ...
    banner.className = 'catpaw-overlay catpaw-banner';
    ...
    if (!EXTENSION_MODE) {
      ...
      toggle.addEventListener('click', () => {
        ...
        document.body.classList.toggle('catpaw-hidden', !overlaysVisible);
  ...
  function buildSelectorSegment(el) {
    ...
    if (el.classList && el.classList.length > 0) {
      const classes = [...el.classList]
        .filter(c => !c.startsWith('catpaw-') && !isLikelyHashedClass(c))
        .slice(0, 2);
      if (classes.length > 0) {
  ...
  function collectVisualContrastCandidates(options = {}) {
    ...
    for (const el of document.querySelectorAll('*')) {
      ...
      if (el.closest('.catpaw-overlay, .catpaw-label, .catpaw-banner, .catpaw-tooltip')) continue;
      if (el.closest('[id^="catpaw-live-"]')) continue;
  ...
  async function sampleVisualBackgroundAtPoint(el, point, textColor, depth = 0) {
    ...
    for (const node of nodes) {
      ...
      if (node.closest?.('.catpaw-overlay, .catpaw-label, .catpaw-banner, .catpaw-tooltip')) continue;
  ...
  const printSummary = function(allFindings) {
    if (allFindings.length === 0) {
      console.log('%c[catpaw] No anti-patterns found.', 'color: #22c55e; font-weight: bold');
      return;
    }
    console.group(
      `%c[catpaw] ${allFindings.length} anti-pattern${allFindings.length === 1 ? '' : 's'} found`,
  ...
  function browserDesignSystemConfig() {
    const raw = window.__CATPAW_CONFIG__?.designSystem;
  ...
  function collectBrowserFindings() {
    ...
    const _disabled = EXTENSION_MODE ? (window.__CATPAW_CONFIG__?.disabledRules || []) : [];
    ...
    for (const el of document.querySelectorAll('*')) {
      // Skip catpaw's own elements and any descendants (overlays, labels, banner, nav buttons)
      if (el.closest('.catpaw-overlay, .catpaw-label, .catpaw-banner, .catpaw-tooltip')) continue;
      ...
      // Skip the catpaw live-mode overlay (highlight, tooltip, bar, picker, toast).
    ...
    // Clone the document and strip catpaw-live overlay nodes before the
    ...
    for (const node of docClone.querySelectorAll('[id^="catpaw-live-"]')) {
      node.remove();
    }
  ...
  function shouldRunVisualContrast(options = {}) {
    return options.visualContrast === true || window.__CATPAW_CONFIG__?.visualContrast === true;
  }

  function visualContrastOptions(options = {}) {
    const config = window.__CATPAW_CONFIG__ || {};
  ...
  function reportVisualContrastError(err, detail = {}) {
    window.dispatchEvent(new CustomEvent('catpaw-visual-contrast-error', {
      detail: {
        ...detail,
        message: err?.message || String(err),
      },
    ...
    if (EXTENSION_MODE) {
      ...
      console.warn('[catpaw] visual contrast scan failed', err);
    }
  }
        ...
        waitForVisualPaint()
          ...
          .then(result => {
            ...
            if (added) {
              ...
              window.dispatchEvent(new CustomEvent('catpaw-visual-contrast-resolved', {
                detail: {
  ...
  if (EXTENSION_MODE) {
    ...
    window.addEventListener('message', (e) => {
      if (e.source !== window || !e.data || e.data.source !== 'catpaw-command') return;
      if (e.data.action === 'scan') {
        if (e.data.config) window.__CATPAW_CONFIG__ = e.data.config;
        try {
          scan(e.data.config || {});
        } catch (err) {
          postExtensionError(err);
      ...
      if (e.data.action === 'toggle-overlays') {
        const visible = !document.body.classList.contains('catpaw-hidden');
        document.body.classList.toggle('catpaw-hidden', visible);
        window.postMessage({ source: 'catpaw-overlays-toggled', visible: !visible }, '*');
      }
      if (e.data.action === 'remove') {
        clearOverlays();
        styleEl.remove();
        if (spotlightBackdrop) { spotlightBackdrop.remove(); spotlightBackdrop = null; }
        document.body.classList.remove('catpaw-hidden');
      }
      if (e.data.action === 'highlight') {
        try {
          ...
          if (target) {
            ...
            for (const o of overlays) {
              if (o.classList.contains('catpaw-banner')) continue;
              const isMatch = o._targetEl === target;
              o.classList.toggle('catpaw-spotlight', isMatch);
              o.classList.toggle('catpaw-spotlight-dimmed', !isMatch);
              if (isMatch) {
                // Force the matching overlay visible immediately, don't wait for IntersectionObserver
                o.style.display = '';
                o.style.animation = 'none';
                o.classList.add('catpaw-visible');
                o._revealed = true;
                positionOverlay(o);
              }
            }
      ...
      if (e.data.action === 'unhighlight') {
        hideSpotlight();
        for (const o of overlays) {
          o.classList.remove('catpaw-spotlight');
          o.classList.remove('catpaw-spotlight-dimmed');
        }
      }
    });
    window.postMessage({ source: 'catpaw-ready' }, '*');
  } else {
    if (window.__CATPAW_CONFIG__?.autoScan !== false) {
      const runAutoScan = () => {
        try {
          scan();
        } catch (err) {
          console.warn('[catpaw] scan failed', err);
        }
      };
  ...
  window.catpawDetect = detect;
  window.catpawDetectAsync = detectAsync;
  window.catpawScan = scan;
  window.catpawScanAsync = scanAsync;
  ...
  window.catpawMeasureHiddenText = measureHiddenTextDOM;
  window.catpawCollectVisualContrastCandidates = collectVisualContrastCandidates;
  window.catpawAnalyzeVisualContrast = analyzeVisualContrast;
  window.catpawGetLastVisualContrastAnalyses = () => lastVisualContrastAnalyses.slice();
}
