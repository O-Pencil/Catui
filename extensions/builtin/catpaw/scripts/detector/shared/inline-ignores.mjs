 ...
 * an ignore in `.catpaw/config.json`.
 *
 ...
 * line, so the same marker works across every comment style catpaw scans —
 * `//`, `/* *\/`, `<!-- -->`, `#`, `{/* *\/}`, `{# #}`. Trailing comment closers
 * are stripped before the rule list is parsed.
 *
 * Syntax (reason optional; eslint `--` or biome `:` separator):
 *
 *   catpaw-disable <rule>[, <rule>...] [-- reason]   whole file
 *   catpaw-disable-line <rule>...      [-- reason]   the same line
 *   catpaw-disable-next-line <rule>... [-- reason]   the following line
 *   catpaw-disable                                   bare / `*` = every rule
 *
 * Examples:
 *
 *   <!-- catpaw-disable overused-font -- exported brand doc, font is first-party -->
 *   .brand { font-family: Inter; } /* catpaw-disable-line overused-font *\/
 *   // catpaw-disable-next-line bounce-easing: intentional playful affordance
 *
...
const DIRECTIVE_RE = /catpaw-(disable-next-line|disable-line|disable)\b[ \t]*([^\n\r]*)/gi;
...
function parseInlineIgnores(content) {
  ...
  // Case-insensitive to match DIRECTIVE_RE's `i` flag (e.g. `Catpaw-Disable`).
  if (!/catpaw-disable/i.test(text)) return result;
