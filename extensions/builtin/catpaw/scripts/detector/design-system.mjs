...
// boundaries; `.catpaw` is our own project marker.
const PROJECT_ROOT_MARKERS = ['.git', 'package.json', '.catpaw'];
...
function resolveDesignSidecarPath(cwd = process.cwd(), contextDir = cwd) {
  const candidates = [
    path.join(cwd, '.catpaw', 'design.json'),
...
//   - A directory carrying a project marker (.git / package.json / .catpaw)
