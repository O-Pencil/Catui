...
async function captureVisualContrastCandidate(page, candidate, viewport) {
  ...
  const token = `catpaw-contrast-${Date.now()}-${Math.random().toString(36).slice(2)}`;
  const applied = await page.evaluate(({ selector, token, backgroundClipText }) => {
    ...
    if (!el) return false;
    let style = document.getElementById('catpaw-visual-contrast-hide-style');
    if (!style) {
      style = document.createElement('style');
      style.id = 'catpaw-visual-contrast-hide-style';
      style.textContent = [
        '[data-catpaw-visual-contrast-target] {',
        '  color: transparent !important;',
        '  -webkit-text-fill-color: transparent !important;',
        '  text-shadow: none !important;',
        '}',
        '[data-catpaw-visual-contrast-target][data-catpaw-bgclip-text="true"] {',
        '  background-image: none !important;',
        '}',
      ].join('\n');
      document.head.appendChild(style);
    }
    el.setAttribute('data-catpaw-visual-contrast-target', token);
    if (backgroundClipText) el.setAttribute('data-catpaw-bgclip-text', 'true');
    return true;
  }, {
  ...
  try {
    ...
    await page.evaluate(({ selector }) => {
      try {
        ...
        if (el) {
          el.removeAttribute('data-catpaw-visual-contrast-target');
          el.removeAttribute('data-catpaw-bgclip-text');
        }
      } catch {
