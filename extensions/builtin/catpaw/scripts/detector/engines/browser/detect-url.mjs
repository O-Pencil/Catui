...
async function measureContentHiddenAfterReveal(page) {
  ...
  return page.evaluate(() => {
    if (typeof window.catpawMeasureHiddenText !== 'function') return null;
    return window.catpawMeasureHiddenText();
  });
}
...
async function runVisualContrastFallback(page, serializedGroups, options, profile, target) {
  ...
  if (options?.visualContrastBrowser !== false) {
    const browserFindings = await profileFindingsAsync(profile, {
      ...
      browserAnalyses = await page.evaluate(async ({ maxCandidates, scrollOffscreen }) => {
        if (typeof window.catpawAnalyzeVisualContrast !== 'function') return [];
        return window.catpawAnalyzeVisualContrast({ maxCandidates, scrollOffscreen });
  ...
  if (candidates.length === 0) {
    candidates = await profileStepAsync(profile, {
      ...
      if (typeof window.catpawCollectVisualContrastCandidates !== 'function') return [];
      return window.catpawCollectVisualContrastCandidates({ maxCandidates });
    }, { maxCandidates }));
  }
  ...
  try {
    ...
    results = await profileFindingsAsync(profile, {
      ...
      serializedGroups = await page.evaluate(() => {
        if (!window.catpawDetect) return [];
        return window.catpawDetect({ decorate: false, serialize: true });
      });
