...
function printUsage() {
  console.log(`Usage: catpaw detect [options] [file-or-dir-or-url...]
...
Options:
  ...
  --no-inline-ignores Do not honor in-file catpaw-disable* ignore comments
  --no-design-system  Do not load local DESIGN.md / .catpaw/design.json context
...
Project config:
  Respects .catpaw/config.json and .catpaw/config.local.json detector
  settings: detector.ignoreRules, detector.ignoreFiles, detector.ignoreValues,
  and detector.designSystem.enabled.

Inline ignores:
  In-file comments waive a finding where it lives and travel with the file:
    <!-- catpaw-disable overused-font -- exported brand doc -->
    .brand { font-family: Inter } /* catpaw-disable-line overused-font */
    // catpaw-disable-next-line bounce-easing: intentional bounce
  catpaw-disable applies to the whole file; -line / -next-line are scoped.
...
Examples:
  catpaw detect src/
  catpaw detect index.html
  catpaw detect https://example.com
  catpaw detect --json .
  catpaw detect --no-config src/`);
}
...
async function detectCli() {
  ...
  // Inline `catpaw-disable*` waivers are part of the scanned file, so they
            ...
            if (fwConfig) {
              ...
              if (probe.listening && probe.matched) {
                process.stderr.write(
                  ...
                  `  npx catpaw detect http://localhost:${fwConfig.port}\n\n`
                );
