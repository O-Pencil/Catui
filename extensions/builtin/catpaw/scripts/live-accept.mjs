...
import { getLiveDir, safeSessionId } from './lib/catpaw-paths.mjs';
 ...
 * <div data-catpaw-carbonize> so the slot keeps a single root node.
 */
function buildCarbonizeReplacement({
  ...
  const pushCarbonizeBody = (bodyIndent) => {
    ...
    lines.push(bodyIndent + commentSyntax.open + ' catpaw-carbonize-start ' + id + ' ' + commentSyntax.close);
    lines.push(bodyIndent + '<style data-catpaw-css="' + id + '">' + (isJsx ? '{`' : ''));
    ...
    if (paramValues && Object.keys(paramValues).length > 0) {
      lines.push(
        bodyIndent + commentSyntax.open + ' catpaw-param-values ' + id + ': ' + JSON.stringify(paramValues) + ' ' + commentSyntax.close,
      );
    }
    lines.push(bodyIndent + commentSyntax.open + ' catpaw-carbonize-end ' + id + ' ' + commentSyntax.close);
    lines.push(bodyIndent + '<div data-catpaw-variant="' + variantNum + '" ' + variantStyleAttr + '>');
  ...
  if (isJsx) {
    ...
    lines.push(indent + '<div data-catpaw-carbonize="' + id + '" ' + wrapperStyle + '>');
...
function buildAcceptedWrappedSource(id, variantNum, lines, targetFile, paramValues) {
  ...
  const hasHelperAttrs = variantText.includes('data-catpaw-variant');
...
function readSourceShadowPreviewMeta(content, id) {
  ...
  const wrapperRe = new RegExp('<[^>]+data-catpaw-variants=(["\'])' + escaped + '\\1[^>]*>');
  const match = String(content || '').match(wrapperRe);
  if (!match) return null;
  const tag = match[0];
  if (readHtmlAttr(tag, 'data-catpaw-preview') !== 'source-shadow') return null;
  const sourceFile = readHtmlAttr(tag, 'data-catpaw-source-file');
  const sourceStartLine = Number(readHtmlAttr(tag, 'data-catpaw-source-start'));
  const sourceEndLine = Number(readHtmlAttr(tag, 'data-catpaw-source-end'));
...
function findMarkerBlock(id, lines) {
  let start = -1;
  let end = -1;
  const startPattern = 'catpaw-variants-start ' + id;
  const endPattern = 'catpaw-variants-end ' + id;
 ...
 * the `<div data-catpaw-variants="ID">` outer wrapper so the picked
 ...
 * (with `data-catpaw-variants="ID"`) and its matching `</div>`. We
...
function expandReplaceRange(block, lines, isJsx) {
  ...
  // Walk back for the wrapper `<div data-catpaw-variants="..."` opener.
...
function isVariantEndMarkerLine(line, id) {
  return new RegExp('catpaw-variants-end\\s+' + escapeRegExp(id) + '(?:\\s|--|\\*/|$)').test(line);
}

function hasVariantWrapperAttr(line, id) {
  const escaped = escapeRegExp(id);
  return new RegExp(`data-catpaw-variants\\s*=\\s*(?:"${escaped}"|'${escaped}'|\\{["']${escaped}["']\\})`).test(line);
}
 ...
 *   - CSS `@scope ([data-catpaw-variant="N"])` strings that look like the
...
function extractOriginal(lines, block) {
  ...
  const inner = extractInnerByAttr(text, 'data-catpaw-variant="original"');
...
function extractVariant(lines, block, variantNum) {
  const text = stripStyleAndJoin(lines, block);
  const inner = extractInnerByAttr(text, 'data-catpaw-variant="' + variantNum + '"');
 ...
 *
 * Handles three shapes of `<style data-catpaw-css="ID" ...>`:
...
function extractCss(lines, block, id) {
  const styleAttr = 'data-catpaw-css="' + id + '"';
  let inStyle = false;
  const content = [];
...
function findSessionFile(id, cwd) {
  const result = findSourceFile({
    query: 'catpaw-variants-start ' + id,
    cwd,
