 ...
 *      `.catpaw/config.json` declares `projectRoots`)
 *   4. $CATPAW_CONTEXT_DIR (absolute or cwd-relative) — power-user
...
import { CATPAW_COMMAND, CATPAW_PROVIDER_ID } from './lib/provider.mjs';
...
// can offer `npx catpaw update`. Everything here is best-effort and
// silent on failure: a network problem, sandbox, or missing cache must never
// block context output or print an error.

const UPDATE_HOST = (process.env.CATPAW_UPDATE_HOST || 'https://catpaw.style').replace(/\/$/, '');
const UPDATE_CACHE_PATH =
  process.env.CATPAW_UPDATE_CACHE || path.join(os.homedir(), '.catpaw', 'update-check.json');
...
function resolveEnvContextDir(cwd) {
  const envDir = process.env.CATPAW_CONTEXT_DIR;
...
// outside both the project and repo roots (CATPAW_CONTEXT_DIR override).
...
// itself. Catpaw `projectRoots` patterns govern every path they match:
...
// deeper folder would silently resolve there. Paths the Catpaw group does
...
function isSelectableCandidate(repoRoot, rel, patternGroups) {
  ...
  const [catpawPatterns, packagePatterns] = patternGroups;
  if (isExcludedByWorkspacePattern(relSegments, catpawPatterns)) return false;
  for (const pattern of catpawPatterns) {
...
// `projectRoots` globs in .catpaw config, then package-manager workspace
// declarations. A path matched by any Catpaw pattern — positive or
// negated — is governed by the Catpaw group alone; package-manager
// patterns only apply to paths the Catpaw group does not match. Within a
// group, negations win over positives.
function readProjectPatternGroups(repoRoot) {
  return [
    readCatpawProjectRoots(repoRoot),
    [
...
function readCatpawProjectRoots(repoRoot) {
  const patterns = [];
  for (const name of ['config.json', 'config.local.json']) {
    const cfg = readJson(path.join(repoRoot, '.catpaw', name));
...
function buildUpdateDirective(localVersion, latestVersion) {
  return (
    `UPDATE_AVAILABLE: A newer Catpaw skill is available ` +
    `(installed v${localVersion}, latest v${latestVersion}). ` +
    `Before continuing, ask the user once: "A newer Catpaw (v${latestVersion}) is available. ` +
    `Update now? It runs \`npx catpaw update\`." ` +
    `If they agree, run \`npx catpaw update\` (the update applies to the next session, not this one). ` +
 ...
 * the agent never nags. Opt out entirely with CATPAW_NO_UPDATE_CHECK=1.
 */
...
function updateCheckDisabledByConfig(cwd = process.cwd()) {
  ...
  for (const name of ['config.json', 'config.local.json']) {
    try {
      const raw = JSON.parse(fs.readFileSync(path.join(cwd, '.catpaw', name), 'utf-8'));
...
async function computeUpdateDirective(now = Date.now()) {
  try {
    if (process.env.CATPAW_NO_UPDATE_CHECK) return null;
  ...
  if (!ctx.hasProduct) {
    ...
    const parts = ctx.hasVisualImplementation
      ? [
          ...
          `offer \`${CATPAW_COMMAND} init\` as a follow-up.`,
      ...
      : [
          ...
          `context and offer \`${CATPAW_COMMAND} init\` as a suggestion (do not block).`,
...
const HOOK_MANIFESTS_BY_PROVIDER = Object.freeze({
  ...
  github: ['.github/hooks/catpaw.json'],
  grok: ['.grok/hooks/catpaw.json'],
});
...
function valueHasHookMarker(value) {
  if (typeof value === 'string') {
    return value.includes('skills/catpaw/scripts/hook.mjs')
      || value.includes('skills/catpaw/scripts/hook-before-edit.mjs');
  }
...
function hookEnabledAt(root) {
  if (truthyEnv(process.env.CATPAW_HOOK_DISABLED)) return false;
  let enabled = true;
  for (const name of ['.catpaw/config.json', '.catpaw/config.local.json']) {
...
function automaticHookMode(ctx) {
  ...
  const manifests = HOOK_MANIFESTS_BY_PROVIDER[CATPAW_PROVIDER_ID] || [];
  ...
  for (const root of roots) {
    for (const rel of manifests) {
      ...
      if (raw?.hooks && valueHasHookMarker(raw.hooks)) {
        return STOP_REVIEW_PROVIDERS.has(CATPAW_PROVIDER_ID) ? 'stop' : 'per-edit';
      }
    }
  }
  return 'none';
...
function appendAutonomyCounterDirective(parts) {
  parts.push([
    ...
    "Catpaw's interview and decision steps stay live: probe once with the structured question tool or the decision page.",
...
function appendDetectorFallback(parts, ctx) {
  ...
  const scriptsPath = path.dirname(fileURLToPath(import.meta.url));
  parts.push([
    'MANUAL_DETECTOR_REQUIRED: No automatic Catpaw design hook is active this session.',
...
// Tier 1 staleness: schema drift in Catpaw's own project files, measured
...
function projectRootsDiagnostic(ctx, options) {
  ...
  const patterns = readCatpawProjectRoots(ctx.repoRoot);
...
function buildTargetSelectionDirective(selection) {
  return (
    ...
    'Ask the user which app Catpaw should use, then rerun Catpaw helper commands from that child app cwd using this same scripts directory. ' +
