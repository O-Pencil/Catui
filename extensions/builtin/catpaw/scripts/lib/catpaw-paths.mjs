...
export { CATPAW_COMMAND_PREFIX } from './provider.mjs';

export const CATPAW_DIR = '.catpaw';
export const LIVE_DIR = 'live';
export const CRITIQUE_DIR = 'critique';

export function getCatpawDir(cwd = process.cwd(), options = {}) {
  return path.join(resolveProjectRoot(cwd, options), CATPAW_DIR);
}

export function getDesignSidecarPath(cwd = process.cwd(), options = {}) {
  return path.join(getCatpawDir(cwd, options), 'design.json');
}
...
export function getLiveDir(cwd = process.cwd(), options = {}) {
  return path.join(getCatpawDir(cwd, options), LIVE_DIR);
}
...
export function resolveLiveConfigPath({ cwd = process.cwd(), scriptsDir, env = process.env, targetPath } = {}) {
  if (env.CATPAW_LIVE_CONFIG && env.CATPAW_LIVE_CONFIG.trim()) {
    const configured = env.CATPAW_LIVE_CONFIG.trim();
...
export function getLegacyLiveServerPath(cwd = process.cwd(), options = {}) {
  return path.join(resolveProjectRoot(cwd, options), '.catpaw-live.json');
}
 ...
 * `.catpaw/live/`. Real IDs are 8 hex chars; the tests use short slugs.
 */
...
export function getLegacyLiveSessionsDir(cwd = process.cwd(), options = {}) {
  return path.join(resolveProjectRoot(cwd, options), '.catpaw-live', 'sessions');
}
...
export function getCritiqueDir(cwd = process.cwd(), options = {}) {
  return path.join(getCatpawDir(cwd, options), CRITIQUE_DIR);
}

export function getLegacyLiveAnnotationsDir(cwd = process.cwd(), options = {}) {
  return path.join(resolveProjectRoot(cwd, options), '.catpaw-live', 'annotations');
}
