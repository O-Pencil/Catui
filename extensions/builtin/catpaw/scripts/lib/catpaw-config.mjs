/**
 * CLI-side reader/writer for the unified `.catpaw` config.
 *
...
export function getConfigPath(root) {
  return join(root, '.catpaw', 'config.json');
}

export function getLocalConfigPath(root) {
  return join(root, '.catpaw', 'config.local.json');
}
...
function applyDetectionConfigSource(config, raw) {
  ...
  // so config round-trips (e.g. `catpaw hooks ignore-value`) preserve it.
...
/**
 * Detector filters shared by `npx catpaw detect` and the design hook.
...
const EXCLUDE_OPEN = '# catpaw-config-ignore-start';
const EXCLUDE_CLOSE = '# catpaw-config-ignore-end';
const EXCLUDE_PATTERNS = ['.catpaw/config.local.json'];
