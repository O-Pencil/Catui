/**
 * Schema versions for the artifacts Catpaw writes, plus the readers and
...
/** `.catpaw/design.json`, as documented in reference/document.md Step 4b. */
...
/**
 * Headings Catpaw used to read and no longer does, with the reason. The
...
const PRODUCT_STAMP_RE = /^[ \t]*<!--[ \t]*catpaw:product-schema[ \t]+(\d+)[ \t]*-->[ \t]*$/im;
...
export function productStampLine(version = PRODUCT_SCHEMA_VERSION) {
  return `<!-- catpaw:product-schema ${version} -->`;
}
