 ...
 *
 * The boot tier answers "did an older Catpaw write this". This tier also
...
// Retired live-mode state locations. catpaw-paths still reads these as
// fallbacks; reporting them is what eventually lets the fallbacks go.
const LEGACY_LIVE_PATHS = ['.catpaw-live.json', '.catpaw-live'];
...
const SEED_DESIGN_MARKERS = ['/', '$'].map((prefix) =>
  '<!-- SEED: established with the user before implementation; '
    + `re-run ${prefix}catpaw document once there's code to capture the actual tokens and components. -->`
);
...
export function checkDetectorIgnores({ projectRoot, knownRuleIds = null }) {
  ...
  for (const name of ['config.json', 'config.local.json']) {
    const filePath = path.join(projectRoot, '.catpaw', name);
...
const HOOK_MARKER = /skills\/catpaw\/scripts\/hook(?:-before-edit)?\.mjs/;
...
function hookScriptTokenFrom(command) {
  ...
  const quoted = str.match(/"([^"]*skills\/catpaw\/scripts\/hook(?:-before-edit)?\.mjs)"/);
  if (quoted) return quoted[1];
  const bare = str.match(/([^\s"'|&;()]*skills\/catpaw\/scripts\/hook(?:-before-edit)?\.mjs)/);
...
export function checkHookInstallation({ projectRoot, repoRoot, providerId }) {
    ...
    for (const rel of manifests) {
      ...
      if (broken.length) {
        findings.push(finding({
          ...
          fix: `Reinstall with \`catpaw hooks on\`, which rewrites the manifest against the skill's current location.`,
        }));
      }
    }
  }
  ...
  if (installedAt) {
    for (const root of roots) {
      for (const name of ['config.json', 'config.local.json']) {
        const raw = readJson(path.join(root, '.catpaw', name));
        if (raw?.hook && raw.hook.enabled === false) {
          findings.push(finding({
            id: 'hook-enabled-conflict',
            artifact: 'config.json',
            filePath: toRelative(path.join(root, '.catpaw', name), projectRoot || root),
            severity: 'mention',
            ...
            fix: 'Ask which was intended: `catpaw hooks on` to enable, or `catpaw hooks off` to uninstall '
...
export function checkLegacyLiveState({ projectRoot }) {
  ...
  return [finding({
    ...
    summary: `Live-mode state sits in retired location(s): ${present.map((rel) => `\`${rel}\``).join(', ')}. `
      + 'Current live mode writes under `.catpaw/live/`.',
