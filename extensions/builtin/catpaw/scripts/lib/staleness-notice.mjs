...
function cachePath() {
  return process.env.CATPAW_STALENESS_CACHE
    || path.join(os.homedir(), '.catpaw', 'staleness-check.json');
}
 ...
 * the file accumulate one entry per directory Catpaw has ever booted in
...
/**
 * Opt out with CATPAW_NO_STALENESS_CHECK=1 or `"stalenessCheck": false` in
 * .catpaw/config.json. Local config overrides shared, matching how
 * updateCheck resolves.
 */
export function stalenessCheckDisabled(roots = [process.cwd()]) {
  if (process.env.CATPAW_NO_STALENESS_CHECK) return true;
  let value;
...
export function buildStalenessDirective(findings) {
  ...
  const lines = [
    ...
    "Catpaw's own project files have drifted from what this version reads. "
