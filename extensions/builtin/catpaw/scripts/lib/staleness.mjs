/**
 * Staleness detection for Catpaw's own project artifacts: PRODUCT.md,
 * DESIGN.md and its `.catpaw/design.json` sidecar, `.catpaw/config.json`,
 ...
 *   2. Schema drift. An artifact was written by an older Catpaw: fields it
 ...
 * catpaw-paths (which resolves the project root) and context.mjs (which
 * cannot import catpaw-paths without a cycle) share one definition of
 * where the retired locations are.
 */
export function designSidecarCandidatesFor(projectRoot, contextDir = projectRoot) {
  const candidates = [
    path.join(projectRoot, '.catpaw', 'design.json'),
 ...
 *
 * `sidecarCandidates` comes from catpaw-paths' resolver so this module
...
export function checkConfig({ projectRoot, repoRoot }) {
  ...
  for (const root of roots) {
    for (const name of ['config.json', 'config.local.json']) {
      const filePath = path.join(root, '.catpaw', name);
 ...
 */
export function checkProjectRoots({ patterns = [], candidates = [], configuredIn = '.catpaw/config.json' }) {
