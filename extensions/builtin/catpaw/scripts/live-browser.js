/**
 * Catpaw Live Variant Mode - Browser Script
 *
 * Injected into the user's page via <script src="http://localhost:PORT/live.js">.
 * The server prepends window.__CATPAW_TOKEN__ and window.__CATPAW_PORT__
 * before this code.
 *
...
(function () {
  ...
  if (window.__CATPAW_LIVE_INIT__) return;
  window.__CATPAW_LIVE_INIT__ = true;

  const TOKEN = window.__CATPAW_TOKEN__;
  const PORT = window.__CATPAW_PORT__;
  const APP_ROOT = window.__CATPAW_APP_ROOT__ || null;
  if (!TOKEN || !PORT) {
    window.__CATPAW_LIVE_INIT__ = false; // reset so the real load can init
    return;
  }
  ...
  const PREFIX = 'catpaw-live';
  const CATPAW_COMMAND = (window.__CATPAW_COMMAND_PREFIX__ || '/') + 'catpaw';
  const PICK_CURSOR_STYLE_ID = PREFIX + '-pick-cursor-style';
  const MANUAL_APPLY_STATE_TTL_MS = 15 * 60 * 1000;
  const sessionState = window.__CATPAW_LIVE_SESSION__?.createLiveBrowserSessionState({
    prefix: PREFIX,
    storage: localStorage,
    idFactory: () => crypto.randomUUID().replace(/-/g, '').slice(0, 8),
  });
  if (!sessionState) {
    console.error('[catpaw] live-browser-session.js was not loaded. Live mode cannot start safely.');
    window.__CATPAW_LIVE_INIT__ = false;
    return;
  }
  ...
  // window.__CATPAW_VOCAB__ when it serves /live.js (same injection path as
  ...
  const VOCAB = Array.isArray(window.__CATPAW_VOCAB__) ? window.__CATPAW_VOCAB__ : [];
  const ICONS = {};
  ...
  const SCROLL_ANCHOR_LOCK_ID = 'catpaw-scroll-anchor-lock';
  const VARIANT_STATE_STYLE_ID = 'catpaw-variant-state';
  const DISCARD_STATE_STYLE_ID = 'catpaw-discard-state';
  ...
  const domHelpers = window.__CATPAW_LIVE_DOM__?.createLiveBrowserDomHelpers({
    prefix: PREFIX,
    skipTags: SKIP_TAGS,
    document,
  });
  if (!domHelpers) {
    console.error('[catpaw] live-browser-dom.js was not loaded. Live mode cannot start safely.');
    window.__CATPAW_LIVE_INIT__ = false;
    return;
  }
  const {
  ...
  window.__CATPAW_LIVE_CHROME_CORE__ = {
    version: 1,
    adapter: window.__CATPAW_LIVE_ADAPTER__ || 'dom',
  ...
  function showHighlight(el) {
    ...
    if (el.hasAttribute?.('data-catpaw-insert-placeholder')) return;
  ...
  function onAnnotDown(e) {
    ...
    const resizeEdge = e.target.closest?.('[data-catpaw-placeholder-resize]')?.dataset.catpawPlaceholderResize;
  ...
  function stripManualEditRuntimeState(root) {
    ...
    const nodes = [root, ...root.querySelectorAll('[data-catpaw-editable], [data-catpaw-original-text], [data-catpaw-text-wrap]')];
    for (const node of nodes) {
      const runtimeEditable = node.hasAttribute('data-catpaw-editable')
        || node.hasAttribute('data-catpaw-original-text');
      node.removeAttribute('data-catpaw-editable');
      node.removeAttribute('data-catpaw-original-text');
      node.removeAttribute('data-catpaw-text-wrap');
      if (runtimeEditable) {
  ...
  function selectionTagLabel(el) {
    if (!el) return '';
    if (el.hasAttribute?.('data-catpaw-insert-placeholder')) return 'slot';
  ...
  function applyPlaceholderSizingStyles(placeholder, sizing) {
    placeholder.dataset.catpawPlaceholderWidth = sizing.kind;
  ...
  function materializePlaceholderWidth(placeholder) {
    ...
    const kind = placeholder.dataset.catpawPlaceholderWidth;
    ...
    placeholder.dataset.catpawPlaceholderWidth = 'explicit';
  }
  ...
  function setLiveState(next) {
    state = next;
    window.__CATPAW_LIVE_STATE__ = next;
    syncPageInteractionCursor();
  ...
  function resolveBarAnchor() {
    ...
    if (currentSessionId && (state === 'GENERATING' || state === 'CYCLING')) {
      const wrapper = document.querySelector('[data-catpaw-variants="' + currentSessionId + '"]');
      if (wrapper) {
        const variantCount = wrapper.querySelectorAll('[data-catpaw-variant]:not([data-catpaw-variant="original"])').length;
  ...
  function isInsertGeneratingSession() {
    ...
    const wrapper = document.querySelector('[data-catpaw-variants="' + currentSessionId + '"]');
    return !!wrapper && wrapper.dataset.catpawMode === 'insert';
  }
  ...
  function buildPlaceholderResizeHandles() {
    ...
    for (const spec of specs) {
      ...
      handle.dataset.catpawPlaceholderResize = spec.edge;
  ...
  function createInsertPlaceholder(anchor, position, layoutAxis) {
    ...
    placeholder.setAttribute('data-catpaw-insert-placeholder', 'true');
  ...
  function ensureConfigureInputStyle() {
    ...
    s.textContent =
      '@keyframes catpaw-configure-voice-pulse { 0%, 100% { opacity: 0.55; } 50% { opacity: 1; } }' +
      ...
      '#' + PREFIX + '-configure-voice[data-listening="true"] svg, #' + PREFIX + '-insert-voice[data-listening="true"] svg { animation: catpaw-configure-voice-pulse 1.1s ease-in-out infinite; }' +
    ...
    if (hasParams || paramsPending) {
      ...
      if (paramsPending) {
        const spinner = el('span', {
          ...
          animation: 'catpaw-spin 0.6s linear infinite',
  ...
  function recoverEmptyCycling(reason) {
    ...
    try {
      console.warn('[catpaw] Refusing to render empty variant cycling state:', reason);
  ...
  // variant wrapper:
  //
  //   <div data-catpaw-variant="1"
  //        data-catpaw-params='[{"id":"density","kind":"steps",...}]'>
  //
  ...
  function getVisibleVariantEl() {
    ...
    return wrapper.querySelector('[data-catpaw-variant="' + visibleVariant + '"]');
  }

  function parseVariantParams(variantEl) {
    // Svelte component variants can't carry a `data-catpaw-params` attribute:
    ...
    if (!variantEl) return [];
    const raw = variantEl.getAttribute('data-catpaw-params');
    if (!raw) return [];
    try {
      const parsed = JSON.parse(raw);
      return Array.isArray(parsed) ? parsed : [];
    } catch (err) {
      console.warn('[catpaw] Invalid data-catpaw-params JSON:', err.message);
      return [];
    }
  }
  ...
  function applyParamValue(variantEl, param, value) {
    ...
    // [data-catpaw-component-mount] with no [data-catpaw-variant="N"]
  ...
  function wrapMixedContentTextNodes(rootEl) {
    ...
    if (hasText && hasElement) {
      for (const node of children) {
        if (node.nodeType === 3 && /\S/.test(node.nodeValue || '')) {
          ...
          wrap.dataset.catpawTextWrap = 'true';
    ...
    for (const child of Array.from(rootEl.children)) {
      if (!child.dataset || !child.dataset.catpawTextWrap) {
  ...
  function unwrapMixedContentTextNodes(rootEl) {
    if (!rootEl || rootEl.nodeType !== 1) return;
    const wraps = rootEl.querySelectorAll('[data-catpaw-text-wrap="true"]');
  ...
  function enableInlineEdit(targetEl) {
    ...
    for (const row of rows) {
      ...
      row.el.dataset.catpawEditable = 'true';
      row.el.dataset.catpawOriginalText = row.text;
  ...
  function disableInlineEdit(opts = {}) {
    for (const row of inlineEditRows) {
      ...
      delete row.el.dataset.catpawEditable;
      delete row.el.dataset.catpawOriginalText;
  ...
  function restoreInlineEditDrafts() {
    for (const row of inlineEditRows) {
      if (inlineEditDrafts.has(row.el)) {
        row.el.textContent = row.el.dataset.catpawOriginalText;
      }
    }
  }
  ...
  function documentRefClassSuffix(el) {
    ...
    for (const cls of el.classList) {
      if (!cls || cls.indexOf('catpaw-') === 0) continue;
  ...
  function copyEditLeafContext(el, originalText, newText) {
    ...
    return {
      ...
      id: el.id || null,
      classes: el.classList ? [...el.classList].filter((cls) => cls.indexOf('catpaw-') !== 0) : [],
      originalText,
      newText,
      textContent: (el.textContent || '').slice(0, 500),
  ...
  function nearbyEditableTextsForManualEdit(rows, activeEl, originalText, newText) {
    ...
    for (const row of rows || []) {
      ...
      out.push({
        ...
        classes: row.el?.classList ? [...row.el.classList].filter((cls) => cls.indexOf('catpaw-') !== 0) : [],
        text,
      });
  ...
  async function applyEditing() {
    ...
    try {
      ...
      console.error('[catpaw] manual edit stash failed:', err);
  ...
  function ensureSpinKeyframes() {
    ...
    style.textContent = '@keyframes catpaw-spin { to { transform: rotate(360deg); } }';
    uiAppendStyle(style);
  }
  ...
  async function fetchPendingCount() {
    try {
      ...
      console.warn('[catpaw] failed to fetch pending count:', err);
    }
  }
  ...
  async function onPendingPillClick() {
    ...
    try {
      ...
      if (result.failed && result.failed.length > 0) {
        console.warn('[catpaw] some copy edits failed:', result.failed);
        ...
        if (n > 0) {
          ...
          console.warn('[catpaw] apply returned no verified edits:', result);
          showToast('No edits applied - see console', 4000);
        }
      }
    } catch (err) {
      console.error('[catpaw] commit failed:', err);
  ...
  async function onPendingTrashClick() {
    ...
    try {
      ...
      console.error('[catpaw] discard failed:', err);
  ...
  async function onPendingKeepFixingClick() {
    ...
    try {
      ...
      console.error('[catpaw] repair retry failed:', err);
  ...
  async function onPendingRollbackClick() {
    ...
    try {
      ...
      console.error('[catpaw] manual Apply rollback failed:', err);
  ...
  function restoreDiscardedManualEdits(entries) {
    ...
    if (failures > 0) {
      console.warn('[catpaw] skipped unsafe copy edit DOM restore for', failures, 'edit(s). Refresh to reset the page DOM.');
    }
    return failures;
  }
  ...
  function mixedTextWrapRestoreHint(el) {
    if (!el || !el.dataset || el.dataset.catpawTextWrap !== 'true' || !el.parentElement) return null;
  ...
  function directMixedTextRestoreNodes(parent) {
    return Array.from(parent?.childNodes || []).filter((node) => {
      ...
      return node.nodeType === 1
        && node.dataset
        && node.dataset.catpawTextWrap === 'true'
  ...
  function syncEditBadgeHitProxies() {
    ...
    for (const target of targets) {
      ...
      if (!proxy) {
        ...
        proxy.type = 'button';
        proxy.tabIndex = -1;
        proxy.dataset.catpawEditBadgeProxy = 'true';
  ...
  function initEditBadge() {
    ...
    if (!uiGetById(PREFIX + '-edit-badge-focus-style')) {
      ...
      s.textContent =
        ...
        '[data-catpaw-editable="true"] { outline: none !important; box-shadow: none !important; }' +
        '[data-catpaw-editable="true"]:focus { outline: none !important; box-shadow: none !important; }' +
        '[data-catpaw-editable="true"]:focus-visible { outline: none !important; box-shadow: none !important; }';
      uiAppendStyle(s);
    }
  }
  ...
  function mountedParameterCount() {
    ...
    const wrapper = document.querySelector('[data-catpaw-variants="' + currentSessionId + '"]');
    if (!wrapper) return 0;
    return [...wrapper.querySelectorAll('[data-catpaw-variant]:not([data-catpaw-variant="original"])')]
  ...
  async function showVariantInDOM(sessionId, num) {
    ...
    const wrapper = document.querySelector('[data-catpaw-variants="' + sessionId + '"]');
  ...
  function parseOriginalMarkupElement(originalMarkup) {
    ...
    const doc = parser.parseFromString('<div id="catpaw-anchor">' + originalMarkup + '</div>', 'text/html');
    return doc.getElementById('catpaw-anchor')?.firstElementChild || null;
  }
  ...
  function isUsableInjectionAnchor(el) {
    return !!el
      ...
      && !own(el)
      && !el.closest?.('[data-catpaw-variants]');
  }
  ...
  function waitForVariantAnchorAndRetry({ filePath, sessionId, srcWrapper, checkpointReason }) {
    ...
    const origContent = srcWrapper?.querySelector('[data-catpaw-variant="original"] > :first-child');
    ...
    pendingVariantAnchorRetryObserver = new MutationObserver(() => {
      ...
      const wrapperLanded = !!document.querySelector('[data-catpaw-variants="' + sessionId + '"]');
      if (!wrapperLanded) {
  ...
  async function probePreviewTree(manifest) {
    ...
    try {
      ...
      return { ok: false, tried: err.catpawTriedUrls || candidates };
    }
  }

  function loadSvelteRuntime(runtimeModule, manifest) {
    const modulePath = runtimeModule || '/src/lib/catpaw/__runtime.js';
  ...
  // componentDir (keyed by variant number), because a `data-catpaw-params`
      ...
      if (entry.kind === 'collection' && entry.item && entry.item.rootTag) {
        ...
        values[entry.prop] = matches.map((itemEl, index) => {
          ...
          if (entry.item.keyField && item[entry.item.keyField] == null) {
            item[entry.item.keyField] = 'catpaw-live-' + index;
          }
          return item;
        });
  ...
  function cloneWithoutElements(rootEl, excludedEls) {
    ...
    const MARK = 'data-catpaw-hydration-excluded';
  ...
  async function mountSvelteComponentVariant(variantNum) {
    ...
    try {
      ...
      console.error('[catpaw] Failed to mount component variant ' + variantNum + ' for ' + sessionId + ':', err);
  ...
  async function injectSvelteComponentsFromManifest(manifestPath, sessionId) {
    ...
    try {
      ...
      const existingWrapper = document.querySelector('[data-catpaw-variants="' + sessionId + '"]');
      ...
      if (!liveEl?.parentElement) {
        console.warn('[catpaw] Could not find original element in live DOM.');
        arrivedVariants = availableVariants;
        expectedVariants = Number(manifest.count) || expectedVariants || arrivedVariants;
      ...
      wrapper.dataset.catpawVariants = sessionId;
      wrapper.dataset.catpawVariantCount = String(manifest.count || expectedVariants || 1);
      wrapper.dataset.catpawPreview = componentPreviewMode;
      wrapper.style.display = 'contents';

      const mountTarget = document.createElement('div');
      mountTarget.dataset.catpawComponentMount = sessionId;
      ...
      saveSession();
      if (parameterGenerationState === 'loading') completeParameterPublication();
      console.log('[catpaw] Mounted ' + arrivedVariants + ' ' + manifest.framework + ' component variants.');
    } catch (err) {
      console.error('[catpaw] Failed to mount component-preview variants:', err);
  ...
  function renderMountErrorCard() {
    ...
    retry.dataset.catpawMountRetry = 'true';
  ...
  function abortSvelteComponentInjection(sessionId, details) {
    try {
      if (svelteComponentSession?.sessionId === sessionId) {
        ...
        const orphan = document.querySelector('[data-catpaw-variants="' + sessionId + '"]');
        if (orphan) orphan.remove();
      }
    } catch (err) {
      console.warn('[catpaw] Svelte component abort cleanup failed:', err);
    }
    hideShaderOverlay();
  ...
  function resetSvelteComponentSession(sessionId, message) {
    try {
      ...
      console.warn('[catpaw] Svelte component reset cleanup failed:', err);
    }
    hideShaderOverlay();
    if (variantObserver) { variantObserver.disconnect(); variantObserver = null; }
    if (pendingSvelteComponentRetryObserver) { pendingSvelteComponentRetryObserver.disconnect(); pendingSvelteComponentRetryObserver = null; }
  ...
  function discardOrphanedSession(reason) {
    ...
    if (!sessionId) return;
    console.warn('[catpaw] Discarding orphaned session ' + sessionId + ': ' + reason);
  ...
  function injectVariantsFromSource(filePath, sessionId, opts = {}) {
    ...
    fetch(url)
      ...
      .then(html => {
        ...
        const startMark = '<!-- catpaw-variants-start ' + sessionId + ' -->';
        const endMark = '<!-- catpaw-variants-end ' + sessionId + ' -->';
        ...
        srcWrapper = doc.querySelector('[data-catpaw-variants="' + sessionId + '"]');
        if (!srcWrapper) {
          console.warn('[catpaw] Variant wrapper not found in source file.');
        ...
        const variants = wrapper.querySelectorAll('[data-catpaw-variant]:not([data-catpaw-variant="original"])');
        arrivedVariants = variants.length;
        expectedVariants = parseInt(wrapper.dataset.catpawVariantCount || arrivedVariants);
        if (arrivedVariants <= 0) {
          if (state === 'GENERATING') {
            ...
            if (!opts.generationCompleted) {
              console.log('[catpaw] Source has scaffold but no variants yet; still generating.');
              return;
            }
            ...
            if (attempt < COMPLETED_SOURCE_FALLBACK_RETRIES) {
              console.log('[catpaw] Generation is done but source shows no variants yet; retrying read ('
        ...
        saveSession();
        if (parameterGenerationState === 'loading') completeParameterPublication();
        console.log('[catpaw] Injected ' + arrivedVariants + ' variants from source file.');
      })
      .catch(err => {
        console.error('[catpaw] Failed to fetch source:', err);
  ...
  function updateSelectedElement() {
    ...
    if (svelteComponentSession?.sessionId === currentSessionId) {
      const anchor = resolveSvelteComponentAnchor();
      if (anchor && !anchor.__catpawFrozenAnchor) selectedElement = anchor;
      return;
    }
    const wrapper = document.querySelector('[data-catpaw-variants="' + currentSessionId + '"]');
    if (!wrapper) return;
  ...
  function readVisibleVariantFromDOM(sessionId) {
    ...
    for (const variant of variants) {
      ...
      const idx = parseInt(variant.dataset.catpawVariant || '0', 10);
      if (idx > 0) return idx;
    }
    return 0;
  }
  ...
  function pickVariantContent(wrapper, index) {
    if (!wrapper) return null;
    const variantDiv = wrapper.querySelector('[data-catpaw-variant="' + index + '"]');
  ...
  // single variant (`… > [data-catpaw-variant="N"]`); without it, targets
  // every variant via the bare `[data-catpaw-variant]` attribute.
  function variantStateSelector(sessionId, num) {
    const wrapper = '[data-catpaw-variants="' + sessionId + '"]';
    const variant = num == null
      ? '[data-catpaw-variant]'
      : '[data-catpaw-variant="' + num + '"]';
  ...
  function updateVariantStateStylesheet(sessionId, num) {
    ...
    const hideOthers = variantStateSelector(sessionId)
      + ':not([data-catpaw-variant="' + num + '"]) { ' + VARIANT_HIDE_DECL + ' }';
  ...
  function showOriginalDuringDiscard(sessionId) {
    ...
    const wrapper = '[data-catpaw-variants="' + sessionId + '"]';
    styleEl.textContent = wrapper + ' > [data-catpaw-variant]:not([data-catpaw-variant="original"]) { display:none !important; }\n'
      + wrapper + ' > [data-catpaw-variant="original"] { display:block !important; }';
  }
  ...
  function startScrollLock(sessionId, initialTargetY, initialAnchorTop) {
    ...
    scrollLockObserver = new MutationObserver((mutations) => {
      for (const m of mutations) {
        if (m.target?.closest?.('[data-catpaw-variants="' + sessionId + '"]')) {
          schedule('mutation-in-wrapper');
          return;
        }
        for (const n of m.addedNodes) {
          if (n.nodeType === 1 && (n.matches?.('[data-catpaw-variants="' + sessionId + '"]') || n.querySelector?.('[data-catpaw-variants="' + sessionId + '"]'))) {
  ...
  function startVariantObserver(sessionId) {
    ...
    const obs = new MutationObserver((mutations) => {
      ...
      // Only react to mutations that add nodes with data-catpaw-variant,
      // or mutations inside the variant wrapper. Ignore our own bar/UI changes.
      let dominated = false;
      for (const m of mutations) {
        if (m.target.closest?.('[data-catpaw-variants]')) { dominated = true; break; }
        for (const n of m.addedNodes) {
          if (n.nodeType !== 1) continue;
          // Direct hit: the added node itself is the wrapper or a variant.
          if (n.dataset?.catpawVariants || n.dataset?.catpawVariant) {
          ...
          if (n.querySelector?.('[data-catpaw-variants],[data-catpaw-variant]')) {
            dominated = true; break;
          }
        }
        if (dominated) break;
      ...
      if (selectedElement && !document.body.contains(selectedElement)) {
        const isInsert = wrapper.dataset.catpawMode === 'insert';
        if (isInsert) {
      ...
      const expected = parseInt(wrapper.dataset.catpawVariantCount || '0');
      ...
      if (arrivedVariants > 0) {
        ...
        recoveryWaitingForAnchor = false;
        hideShaderOverlay();
        if (wrapper.dataset.catpawMode === 'insert') finalizeInsertSession();
        updateSelectedElement();
        showOrUpdateCyclingBar();
        disableInlineEdit();
  ...
  function startScrollTracking() {
    function tick() {
      if (state === 'CONFIGURING' || state === 'GENERATING' || state === 'CYCLING') {
        ...
        if (hiTarget && !hiTarget.hasAttribute?.('data-catpaw-insert-placeholder')) {
  ...
  function connectSSE() {
    ...
    evtSource.onmessage = (e) => {
      ...
      switch (msg.type) {
        case 'connected':
          ...
          if (!hasProjectContext) showToast(`No PRODUCT.md found. Variants will be brand-agnostic. Run ${CATPAW_COMMAND} init to generate one.`, 7000);
          console.log('[catpaw] Live mode connected.');
        ...
        case 'error':
          ...
          if (awaitingAcceptResult?.id && msg.id === awaitingAcceptResult.id) {
            awaitingAcceptResult = null;
            console.error('[catpaw] Accept failed after teardown:', msg.message);
          ...
          if (maybeCompleteSteer(msg)) break;
          console.error('[catpaw] Error:', msg.message);
    ...
    evtSource.onerror = () => {
      ...
      if (sseRetries <= SSE_MAX_RETRIES) {
        console.log('[catpaw] SSE connection lost. Retry ' + sseRetries + '/' + SSE_MAX_RETRIES + '...');
        return; // EventSource auto-reconnects
      }
      // Server is gone. Clean up gracefully.
      console.log('[catpaw] Live server unreachable. Cleaning up UI.');
      evtSource.close();
      evtSource = null;
      handleServerLost();
    };
  ...
  function sendEvent(msg, opts) {
    ...
    function handleFailure(err) {
      if (opts && opts.throwOnError) {
        console.error('[catpaw] Failed to send event:', err);
        throw err;
      }
      console.debug('[catpaw] Dropped optional live event:', err);
      return null;
    }
    // Token in the query string as well as the body: the URL token is what
    // authorizes the CORS preflight when the page runs on a non-loopback
  ...
  function abandonForeignSession(sessionId) {
    ...
    console.warn('[catpaw] The live server has no record of session ' + sessionId + '; clearing stale local state.');
    markSessionHandled();
    cleanup({ instantChrome: true });
  ...
  function handleKeyDown(e) {
    ...
    if (e.target.isContentEditable && isInlineEditActive(e.target)) {
      ...
      e.preventDefault();
      e.stopPropagation();
      const original = e.target.dataset.catpawOriginalText;
  ...
  function shouldUseAncestorCropShaderProxy(el) {
    ...
    const adapter = String(window.__CATPAW_LIVE_ADAPTER__ || '').toLowerCase();
    ...
    const wrapper = el?.closest?.('[data-catpaw-variants]');
    return isFrameworkComponentPreviewMode(wrapper?.dataset?.catpawPreview);
  }
    ...
    try {
      ...
      if (shouldUseAncestorCropShaderProxy(el)) {
        try {
          ...
          console.warn('[catpaw] Svelte ancestor crop capture failed, falling back to element capture:', err);
        }
      }
  ...
  async function captureAndEmit(el, basePayload, snapshot, rect) {
    ...
    try {
      ...
      console.warn('[catpaw] capture failed, proceeding without screenshot:', err);
    }
    ...
    if (blob && hasAnnotations) {
      try {
        ...
        if (uploadRes.ok) {
          ...
          console.warn('[catpaw] annotation upload failed:', uploadRes.status);
        }
      } catch (err) {
        console.warn('[catpaw] annotation upload failed:', err);
      }
    }
...
void main() {
  ...
  async function showShaderOverlay(el, blob, rect, paper) {
    ...
    try {
      ...
      console.warn('[catpaw] shader setup failed:', err);
      canvas.remove();
      return;
    }
    ...
    try {
      ...
      console.warn('[catpaw] shader bitmap decode failed:', err);
  ...
  async function handleAccept() {
    ...
    const acceptWrapper = document.querySelector('[data-catpaw-variants="' + currentSessionId + '"]');
    ...
    const acceptedIsSvelteComponent = svelteComponentSession?.sessionId === acceptedSessionId
      || isFrameworkComponentPreviewMode(acceptWrapper?.dataset?.catpawPreview);
    ...
    sendEvent(acceptPayload, { throwOnError: true })
      .then(() => {
        ...
        document.documentElement.dataset.catpawAcceptToPickingMs = String(Date.now() - acceptPayload.clientSentAt);
  ...
  function snapshotAcceptedVariantDom(sessionId, variantId) {
    const wrapper = document.querySelector('[data-catpaw-variants="' + sessionId + '"]');
    const accepted = wrapper?.querySelector?.('[data-catpaw-variant="' + variantId + '"]');
  ...
  function acceptedDomAlreadyClean(pending) {
    ...
    return matches.length > 0
      && matches.every((el) => !el.closest('[data-catpaw-variants],[data-catpaw-variant],[data-catpaw-carbonize]'));
  }
  ...
  function findAcceptedRuntimeWrappers(sessionId) {
    if (!sessionId) return [];
    return [...new Set([
      ...document.querySelectorAll('[data-catpaw-variants="' + sessionId + '"]'),
      ...document.querySelectorAll('[data-catpaw-carbonize="' + sessionId + '"]'),
    ])];
  }
  ...
  function reloadAfterMissingAcceptedDom(pending) {
    if (acceptedDomAlreadyClean(pending)) return;
    if (pending?.id && document.querySelector('[data-catpaw-variants="' + pending.id + '"]')) return;
    location.reload();
  }
  ...
  function commitAcceptedVariantToDom(sessionId, variantId) {
    ...
    const style = wrapper.querySelector('style[data-catpaw-css]');
    if (style && !document.querySelector('style[data-catpaw-accepted-css="' + sessionId + '"]')) {
      const promotedStyle = style.cloneNode(true);
      promotedStyle.setAttribute('data-catpaw-accepted-css', sessionId);
  ...
  function restoreFromActiveSessions(activeSessions, reason) {
    const wrapper = document.querySelector('[data-catpaw-variants]');
    if (wrapper && !isFrameworkComponentPreviewMode(wrapper.dataset.catpawPreview)) return false;
  ...
  function recoverMissedGenerationCompletion(activeSessions) {
    ...
    if (!file) return;
    console.log('[catpaw] Reconnected after generation completed; recovering variants from source.');
    setTimeout(() => {
  ...
  function loadSession() {
    ...
    if (saved?.appRoot && APP_ROOT && saved.appRoot !== APP_ROOT) {
      console.warn('[catpaw] Ignoring saved live session from another project (' + saved.appRoot + ').');
  ...
  function cleanup(options) {
    ...
    if (svelteComponentSession?.sessionId === cleanupSessionId) {
      ...
      const wrapper = document.querySelector('[data-catpaw-variants="' + cleanupSessionId + '"]');
      if (wrapper) {
      ...
      setTimeout(function() {
        ...
        const lateWrapper = document.querySelector('[data-catpaw-variants="' + cleanupSessionId + '"]');
        if (!lateWrapper) return;
        const orig = lateWrapper.querySelector('[data-catpaw-variant="original"]');
        if (orig) {
  ...
  // If a [data-catpaw-variants] wrapper exists in the DOM, the agent wrote
  ...
  function resumeSession() {
    ...
    const sessionId = wrapper.dataset.catpawVariants;
    ...
    // wrapper holds a single mount target, not [data-catpaw-variant] nodes,
    ...
    if (isFrameworkComponentPreviewMode(wrapper.dataset.catpawPreview)
    ...
    if (isFrameworkComponentPreviewMode(wrapper.dataset.catpawPreview)) {
      ...
      currentSessionId = sessionId;
      expectedVariants = Number(wrapper.dataset.catpawVariantCount)
    ...
    currentSessionId = sessionId;
    expectedVariants = parseInt(wrapper.dataset.catpawVariantCount || '0');
    const variants = wrapper.querySelectorAll('[data-catpaw-variant]:not([data-catpaw-variant="original"])');
    arrivedVariants = variants.length;
      ...
      if (shaderTarget) {
        (async () => {
          try {
            ...
            console.warn('[catpaw] shader resume failed:', err);
          }
        })();
      }
    }
  ...
  const PICK_PREFS_KEY = 'catpaw-live-pick';
  const INTERACTION_PREFS_KEY = 'catpaw-live-interaction';
  ...
  function detectPageTheme() {
    try {
      // Dev override: set localStorage 'catpaw-dev-theme' to 'light' or
      // 'dark' to preview the opposite palette without actually changing the
      // page bg. Used for screenshots and theme QA.
      const override = localStorage.getItem('catpaw-dev-theme');
  ...
  function attachSteerFocusGuard() {
    if (window.__CATPAW_STEER_FOCUS_GUARD__) return;
    window.__CATPAW_STEER_FOCUS_GUARD__ = true;
  ...
  function steerFocusDebugEnabled() {
    try { return localStorage.getItem('catpaw-steer-debug') === '1'; } catch { return false; }
  }

  function steerFocusLog(reason, extra) {
    if (!steerFocusDebugEnabled()) return;
    console.log('[catpaw.steer]', reason, {
      state,
      pickActive,
  ...
  function attachSteerFocusDebug() {
    if (!steerFocusDebugEnabled()) return;
    if (window.__CATPAW_STEER_FOCUS_DEBUG__) return;
    window.__CATPAW_STEER_FOCUS_DEBUG__ = true;
  ...
  function buildSteerProcessingDots() {
    ...
    for (let i = 0; i < 3; i++) {
      wrap.appendChild(el('span', {
        ...
        animation: 'catpaw-steer-dot 1.05s ease-in-out ' + (i * 0.14) + 's infinite',
      }));
    }
    return wrap;
  }
  ...
  function startVoice(ctx) {
    ...
    rec.onerror = (event) => {
      ...
      console.warn('[catpaw.voice] recognition error:', code);
    ...
    try {
      ...
      console.warn('[catpaw.voice] start failed:', err);
      stopVoice({
        suppressSubmit: true,
  ...
  function initPageChat(parent, P) {
    ...
    if (!uiGetById(PREFIX + '-page-chat-style')) {
      ...
      s.textContent =
        '@keyframes catpaw-steer-dot { 0%, 70%, 100% { opacity: 0.28; transform: scale(0.82); } 35% { opacity: 1; transform: scale(1); } }' +
        '@keyframes catpaw-steer-processing { 0%, 100% { border-color: oklch(70% 0.12 188 / 0.28); box-shadow: 0 0 0 0 oklch(70% 0.12 188 / 0); } 50% { border-color: oklch(82% 0.07 188 / 0.55); box-shadow: 0 0 14px oklch(70% 0.12 188 / 0.18); } }' +
        '@keyframes catpaw-voice-pulse { 0%, 100% { opacity: 0.55; } 50% { opacity: 1; } }' +
        '#' + PREFIX + '-page-chat[data-processing="true"] { animation: catpaw-steer-processing 1.6s ease-in-out infinite; }' +
        ...
        '#' + PREFIX + '-page-chat-voice[data-listening="true"] svg { animation: catpaw-voice-pulse 1.1s ease-in-out infinite; }' +
  ...
  // Catpaw mark - same paths as site/components/Header.astro + favicon.svg.
  ...
  function syncAgentPollingUi(connected) {
    ...
    globalBarBrandEl.setAttribute('aria-label', agentStatusMessage
      ? 'Catpaw live mode - ' + (agentHasWorkInFlight() ? 'agent is working' : 'agent not polling')
      : 'Catpaw live mode');
  ...
  function initGlobalBar() {
    ...
    if (!uiGetById(PREFIX + '-bar-focus-style')) {
      ...
      s.textContent =
        ...
        '}' +
        '@keyframes catpaw-agent-dot { 0%, 100% { opacity: 0.45; transform: scale(0.9); } 50% { opacity: 1; transform: scale(1); } }' +
        '#' + PREFIX + '-global-bar-brand[data-agent-connected="false"] [data-agent-dot] { animation: catpaw-agent-dot 1.4s ease-in-out infinite; }' +
    ...
    // Brand mark - kinpaku Catpaw icon (site header / favicon paths).
    const brand = el('span', {
      display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
    ...
    brand.setAttribute('aria-label', 'Catpaw live mode - agent not polling');
  ...
  function updateGlobalBarState() {
    ...
    document.querySelectorAll('.catpaw-overlay').forEach(o => {
      o.style.pointerEvents = (pickActive || insertActive) ? 'none' : '';
    });
    syncPageInteractionCursor();
  }

  let detectReady = false; // true once detect script posts 'catpaw-ready'
  ...
  function toggleDetect() {
    ...
    if (detectActive) {
      ...
      window.postMessage({ source: 'catpaw-command', action: 'remove' }, '*');
  ...
  function loadDetectScript() {
    ...
    s.dataset.catpawExtension = 'true';
  ...
  function onDetectMessage(e) {
    ...
    if (e.data.source === 'catpaw-results') {
  ...
  function teardown() {
    ...
    removeVariantStateStylesheet();
    window.__CATPAW_LIVE_INIT__ = false;
    console.log('[catpaw] Live mode exited.');
  }

  //
  // Design System Panel - visualizes the project's .catpaw/design.json sidecar
  //

  const DESIGN_PREFS_KEY = 'catpaw-live-design-panel';
  ...
  let designState = {
    ...
    sidecar: null,          // .catpaw/design.json v2 payload (extensions + components + narrative)
    hasMd: false,
    hasSidecar: false,
  ...
  // Neutral panel palette - deliberately NOT Catpaw-branded. The panel is
  // a viewer of the project's design system, not an Catpaw surface.
  const DP = {
  ...
  function renderDesignBody() {
    ...
    if (designState.present === false) {
      ...
      empty.innerHTML = `<strong>No DESIGN.md yet</strong>Create one by running <code>${CATPAW_COMMAND} document</code> in your terminal, then re-open this panel.`;
  ...
  function renderStaleHint() {
    ...
    box.innerHTML = `
      ...
      <span class="stale-text"><strong>DESIGN.md is newer than .catpaw/design.json.</strong> Run <code>${CATPAW_COMMAND} document</code> to refresh the sidecar.</span>
    `;
    return box;
  }
  ...
  function renderParsedMdCta() {
    ...
    box.innerHTML = `<strong>Basic view</strong>This panel reads the tokens in your <code>DESIGN.md</code> frontmatter. Running <code>${CATPAW_COMMAND} document</code> also generates a <code>.catpaw/design.json</code> sidecar with your project's actual component snippets (button, input, nav) and tonal ramps, rendered live below the tokens.`;
    return box;
  }
  ...
  function designEmptyMessage() {
    if (designState.hasMd && !designState.hasSidecar) {
      return 'DESIGN.md found, no structured tokens to display. Run ' + CATPAW_COMMAND + ' document to generate the .catpaw/design.json sidecar.';
    }
  ...
  function cssSafe(v) {
    ...
    // .catpaw/design.json values rendered into inline style strings.
  ...
  function init() {
    ...
    if (!resumeSession()) {
      console.log('[catpaw] Live variant mode ready. Hover over elements to pick one.');
      ...
      const scout = new MutationObserver(() => {
        ...
        scout.disconnect();
        if (resumeSession()) {
          console.log('[catpaw] Resumed deferred session ' + currentSessionId + ' (post-hydration).');
        }
      });
      scout.observe(document.body, { childList: true, subtree: true });
    } else {
      console.log('[catpaw] Resumed active variant session ' + currentSessionId + ' (' + arrivedVariants + '/' + expectedVariants + ' variants).');
    }
