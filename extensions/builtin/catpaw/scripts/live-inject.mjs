 ...
 *
 * On first live run, the agent generates `.catpaw/live/config.json`
...
import { resolveLiveConfigPath } from './lib/catpaw-paths.mjs';
import {
...
const IGNORE_MARKER_OPEN = '# catpaw-live-ignore-start';
const IGNORE_MARKER_CLOSE = '# catpaw-live-ignore-end';

export const LIVE_IGNORE_PATTERNS = Object.freeze([
  '.catpaw/hook.cache.json',
  '.catpaw/hook.pending.json',
  '.catpaw/config.local.json',
  '.catpaw/live/server.json',
  '.catpaw/live/roots.json',
  '.catpaw/live/app-root.json',
  '.catpaw/live/inject-journal.json',
  '.catpaw/live/sessions/',
  '.catpaw/live/previews/',
  '.catpaw/live/annotations/',
  '.catpaw/live/artifacts/',
  '.catpaw/live/accept-receipts/',
  '.catpaw/live/locks/',
  '.catpaw/live/cache/',
  '.catpaw/live/manual-edit-apply-transaction.json',
  '.catpaw/live/manual-edit-events.jsonl',
  '.catpaw/live/manual-edit-evidence/',
  '.catpaw/live/pending-manual-edits.json',
  '.catpaw/live/deferred-svelte-component-accepts.json',
  '.catpaw-live.json',
  '.catpaw-live/',
  'app/.catpaw-live/',
  'src/.catpaw-live/',
  'node_modules/.catpaw-live/',
  'src/lib/catpaw/CatpawLiveRoot.svelte',
  'src/lib/catpaw/__runtime.js',
  'src/lib/catpaw/[0-9a-f]*/',
  'plugins/catpaw-live.client.ts',
  'app/plugins/catpaw-live.client.ts',
  'src/plugins/catpaw-live.client.ts',
]);
...
Reads configuration from .catpaw/live/config.json.

Modes:
  --port PORT   Insert script tag pointing at http://localhost:PORT/live.js
  --remove      Remove the script tag (if present)
  --check       Print whether .catpaw/live/config.json exists and its content
...
Output (JSON):
  ...
  if (!token) {
    try {
      const info = JSON.parse(fs.readFileSync(path.join(cwd, '.catpaw', 'live', 'server.json'), 'utf-8'));
  ...
  try {
    const rootsManifest = JSON.parse(fs.readFileSync(path.join(cwd, '.catpaw', 'live', 'roots.json'), 'utf-8'));
