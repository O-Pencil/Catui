 ...
 * service repo, evals, and tests set CATPAW_CATALOG_DIR), then the roll
 * API at catpaw.style, then a degraded assignment-only seed when both are
 * unavailable. --chosen sends the anonymous choice ping for API-dealt rolls;
 * DO_NOT_TRACK or CATPAW_NO_TELEMETRY disables it.
 *
 * Env vars:
 *   CATPAW_CONCEPT_SEED — same as --from; for reproducible eval runs.
 *   CATPAW_CATALOG_DIR  — directory holding the four catalog JSON files.
 *   CATPAW_API_URL      — roll API base (default https://catpaw.style/api).
 *   CATPAW_NO_TELEMETRY — disables the choice ping (DO_NOT_TRACK also honored).
 */
...
// tests point CATPAW_CATALOG_DIR at one), then the roll API, then a
// degraded assignment-only seed. The full catalog does not ship with the skill.
const CATALOG_DIR = process.env.CATPAW_CATALOG_DIR || here;
const API_BASE = (process.env.CATPAW_API_URL || 'https://catpaw.style/api').replace(/\/$/, '');
const API_TIMEOUT_MS = Number(process.env.CATPAW_API_TIMEOUT || 4000);
...
function requireLocalConcepts() {
  ...
  if (!local) {
    throw new Error('concept-seed: no local catalog (set CATPAW_CATALOG_DIR or pass sourceConcepts)');
  }
  return local;
}
...
function telemetryDisabled() {
  return Boolean(process.env.CATPAW_NO_TELEMETRY || process.env.DO_NOT_TRACK);
}
...
const CARD_BASE = process.env.CATPAW_CARD_BASE || 'https://catpaw.style/worlds/cards';
...
export function renderConceptSeed({
  scope = 'surface',
  key = process.env.CATPAW_CONCEPT_SEED || crypto.randomBytes(4).toString('hex'),
  reroll = 0,
  mode = null,
  grain = null,
  platform = null,
...
network contact is one GET to https://catpaw.style/api/roll whose query
...
`;
  ...
  // widens it. CATPAW_COMPOSITIONS=1 re-enables rendering for catalog
  // development; the draw machinery, axes, and grain report stay intact.
  const compositionsEnabled = process.env.CATPAW_COMPOSITIONS === '1';
...
rivals to your habitual layout, and keep only what makes this product clearer.${grainNote}\n`
  ...
  automatically when DO_NOT_TRACK or CATPAW_NO_TELEMETRY is set.\n`
    : '';
  ...
  try {
    if (chosenIdx !== -1) {
      ...
      process.stdout.write(await renderConceptSeed({
        ...
        key: fromIdx !== -1
          ? args[fromIdx + 1]
          : (process.env.CATPAW_CONCEPT_SEED || crypto.randomBytes(4).toString('hex')),
