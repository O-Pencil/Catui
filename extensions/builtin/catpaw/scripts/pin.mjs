 ...
 *
 * `pin audit` creates a lightweight audit skill that redirects to Catpaw's audit workflow.
...
const PIN_MARKER = '<!-- catpaw-pinned-skill -->';
...
/**
 * Find harness skill directories that have an catpaw skill installed.
 */
function findHarnessDirs(projectRoot) {
  const dirs = [];
  for (const harness of HARNESS_DIRS) {
    const skillsDir = join(projectRoot, harness, 'skills');
    // Only pin in harness dirs that already have catpaw installed
    const catpawDir = join(skillsDir, 'catpaw');
    if (existsSync(catpawDir) || existsSync(join(skillsDir, 'i-catpaw'))) {
      dirs.push(skillsDir);
    }
  }
  return dirs;
...
function generatePinnedSkill(command, metadata, commandPrefix) {
  const desc = metadata[command]?.description || `Shortcut for ${commandPrefix}catpaw ${command}.`;
...
This is a pinned shortcut for \`${commandPrefix}catpaw ${command}\`.

Invoke ${commandPrefix}catpaw ${command}, passing along any arguments provided here, and follow its instructions.
`;
}
...
function pin(command, projectRoot) {
  ...
  if (harnessDirs.length === 0) {
    console.log('No harness directories with catpaw installed found.');
    return false;
  }
...
function unpin(command, projectRoot) {
  ...
  if (removed > 0) {
    ...
    console.log(`Use Catpaw's '${command}' workflow directly to access it.`);
  } else {
