/**
 * Shared library for the Catpaw design hook.
 *
...
export const ENVELOPE_PREFIX = '[catpaw@1]';
...
// behavior with `.catpaw/config.json` → `hook: { "perEditRules": "all" }`.
...
// A project opts back in with `.catpaw/config.json`:
...
export const HOOK_LOCAL_IGNORE_PATTERNS = Object.freeze([
  '.catpaw/hook.cache.json',
  '.catpaw/hook.pending.json',
  '.catpaw/config.local.json',
]);

const HOOK_IGNORE_MARKER_OPEN = '# catpaw-hook-ignore-start';
const HOOK_IGNORE_MARKER_CLOSE = '# catpaw-hook-ignore-end';
...
export function getConfigPath(cwd) {
  return path.join(cwd, '.catpaw', 'config.json');
}

export function getLocalConfigPath(cwd) {
  return path.join(cwd, '.catpaw', 'config.local.json');
}

export function getCachePath(cwd) {
  return path.join(cwd, '.catpaw', 'hook.cache.json');
}

export function getPendingPath(cwd) {
  return path.join(cwd, '.catpaw', 'hook.pending.json');
}
...
function looksLikeProjectRoot(dir) {
  return ['.git', 'package.json', '.catpaw'].some((marker) => {
    try { return fs.existsSync(path.join(dir, marker)); } catch { return false; }
  });
}

// Where `.catpaw/` (cache + config) lives for this event. Normally the
...
// .catpaw), key to the edited file's nearest project root instead, so a
...
export function suppressionNotice(filePath) {
  return `${ENVELOPE_PREFIX} Suppressing further design hints on ${filePath}. More than ${EDIT_COUNT_THRESHOLD} edits in this session reached. Run ${CATPAW_COMMAND} audit to revisit.`;
}
...
export function renderTemplate(findings, filePath, config, opts = {}) {
  ...
  const more = remaining > 0
    ? `... and ${remaining} more (see ${CATPAW_COMMAND} audit).`
    : null;
...
function renderGroupedTemplate(groups, config, opts = {}) {
  ...
  for (const group of realGroups) {
    ...
    if (hidden > 0) {
      lines.push(`- ... ${hidden} more in ${display} (see ${CATPAW_COMMAND} audit).`);
    }
  }
...
function clampGroupedToBudget(header, lines, footer, maxChars) {
  const assemble = (linesArr, omitted) => [
    header,
    ...linesArr,
    ...(omitted ? [`... and more (see ${CATPAW_COMMAND} audit).`] : []),
    '',
    footer,
  ].join('\n');
...
function clampToBudget(header, lines, more, footer, maxChars) {
  ...
  while (assembled.length > maxChars && working.length > 1) {
    working.pop();
    moreText = `... and more (see ${CATPAW_COMMAND} audit).`;
...
function formatFindingIgnoreCommand(finding) {
  ...
  return `${CATPAW_COMMAND} hooks ignore-value ${rule} ${valueArg} --shared --reason ${reason}`;
}
...
export function resolveHarness(env = {}, event = null) {
  const explicit = env?.CATPAW_HOOK_HARNESS;
...
export function writeAuditLog(env, entry, cwd = process.cwd()) {
  ...
  let target = env?.CATPAW_HOOK_LOG;
...
// silence-on-clean can set `CATPAW_HOOK_QUIET=1` — runHook checks that
...
const STEER_LINE = 'That does not mean the design is good: keep following the project design system and the catpaw skill guidance.';
...
export function appendDesignSystemNote(text, scanOptions) {
  ...
  return `${text}\n\n${ENVELOPE_PREFIX} DESIGN.md is newer than .catpaw/design.json. Run ${CATPAW_COMMAND} document to refresh the design-system sidecar.`;
}
...
function directiveFooter(display, opts = {}) {
  ...
  const fileIgnoreGuidance = `run \`${CATPAW_COMMAND} hooks ignore-value <id> "*" --file ${target}\` to scope just that rule to the file, or \`${CATPAW_COMMAND} hooks ignore-file ${target}\` only when the whole file is out of scope for design review (a fixture, a generated artifact, a deliberate demo)`;
  return [
    ...
    '',
    `Do not change intentional design just to satisfy the hook, and do not silence a real finding with an inline ignore comment to skip fixing it. Suppress a finding only after the user explicitly confirms it is intentional. Prefer a config ignore (one reviewable place, the commands below); reach for an inline \`catpaw-disable <rule>\` comment only when the waiver must travel with a file that leaves the repo, such as an exported or standalone document. Prefer the narrowest persisted exception: run the exact \`${CATPAW_COMMAND} hooks ignore-value ... --shared\` command shown next to a value-specific finding. For \`overused-font\`, use \`ignore-value\` for a specific font and use \`${CATPAW_COMMAND} hooks ignore-rule overused-font --all-values\` only when the user asks to ignore overused fonts generally. For a finding whose line shows no exact ignore-value command, such as \`side-tab\`, ${fileIgnoreGuidance}; use \`${CATPAW_COMMAND} hooks ignore-rule <id>\` only when the user asks to suppress the whole non-value-specific rule. Run ${CATPAW_COMMAND} audit for the full pass.`,
  ].join('\n');
}
...
export async function runHook({ stdinJson, env = {}, cwd = process.cwd(), now = Date.now, detector } = {}) {
  ...
  try {
    // Re-entrancy guard.
    if (depthIsSet(env.CATPAW_HOOK_DEPTH) || depthIsSet(env.CLAUDE_HOOK_DEPTH)) {
      return result({ reentrant: true, durationMs: 0 });
    }

    if (truthy(env.CATPAW_HOOK_DISABLED)) {
    ...
    let skippedBytes = 0;
    const quietMode = truthy(env.CATPAW_HOOK_QUIET) || config.quiet === true;
    ...
    // `.catpaw/` (dedup and suppression need it), deferred findings do
    // too (the Stop deep pass needs the touched-file list to surface them),
    // and an already-present `.catpaw/` dir marks a project that opted
    // in. A non-UI edit, or a clean UI edit in a project with no Catpaw
    // footprint, must be a no-op on disk (issues #344, #305).
    if (freshGroups.length > 0 || deferredTotal > 0
      || (cacheDirty && fs.existsSync(path.join(projectCwd, '.catpaw')))) {
