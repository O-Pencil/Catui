...
export function assembleLiveBrowserScript({ token, port, vocabulary, commandPrefix = '/', appRoot = null, parts }) {
  const prelude =
    `window.__CATPAW_TOKEN__ = '${token}';\n` +
    `window.__CATPAW_PORT__ = ${port};\n` +
    ...
    `window.__CATPAW_APP_ROOT__ = ${JSON.stringify(appRoot)};\n` +
    `window.__CATPAW_COMMAND_PREFIX__ = ${JSON.stringify(commandPrefix)};\n` +
    ...
    `window.__CATPAW_VOCAB__ = ${JSON.stringify(vocabulary)};\n`;

  const body = parts.map((part) => {
    const file = part.file || path.basename(part.path || '');
    return `// --- catpaw live script part: ${part.name} (${file}) ---\n${part.source}`;
  }).join('\n');
