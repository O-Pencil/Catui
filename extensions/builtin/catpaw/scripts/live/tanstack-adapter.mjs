 ...
 *
 * The managed component lives OUTSIDE `src/routes/` (in `src/catpaw/`) so
...
export const TANSTACK_MARKER_OPEN = '{/* catpaw-live-tanstack-start */}';
export const TANSTACK_MARKER_CLOSE = '{/* catpaw-live-tanstack-end */}';
export const TANSTACK_COMPONENT_DIR = 'src/catpaw';
export const TANSTACK_COMPONENT_BASENAME = 'CatpawLiveRoot';
...
export function applyTanStackLiveAdapter({ cwd = process.cwd(), port, token, project = detectTanStackStartProject(cwd) } = {}) {
  ...
  if (componentExisted && !isManagedComponent(fs.readFileSync(componentAbs, 'utf-8'))) {
    // A non-Catpaw file already sits at our managed path — refuse to clobber.
    return {
      file: project.componentFile,
      error: 'tanstack_component_conflict',
      hint: `${project.componentFile} already exists and is not managed by Catpaw Live`,
    };
  }
...
export function patchTanStackRoot(content, componentImport) {
  ...
  const importStatement = `import CatpawLiveRoot from '${componentImport}';`;
...
export function unpatchTanStackRoot(content) {
  ...
  out = out.replace(
    new RegExp("^import CatpawLiveRoot from '[^']*';[ \\t]*\\r?\\n", 'gm'),
    '',
  );
  return out;
}
...
export function buildTanStackLiveRootComponent(port, token) {
  ...
  return `/* catpaw-live-tanstack-start */
import { useEffect } from 'react';

const LIVE_SRC = '${liveSrc}';
const LIVE_SELECTOR = 'script[data-catpaw-live-tanstack]';

// Dev-only mount for Catpaw Live. TanStack Start server-renders the root
...
export default function CatpawLiveRoot() {
  useEffect(() => {
    ...
    script.src = LIVE_SRC;
    script.async = true;
    script.setAttribute('data-catpaw-live-tanstack', '');
    script.setAttribute('data-catpaw-live-script', 'true');
...
// The managed mount component carries the `catpaw-live-tanstack` marker in
// its leading comment and its script data-attribute; user files never do.
function isManagedComponent(content) {
  return String(content || '').includes('catpaw-live-tanstack');
}
