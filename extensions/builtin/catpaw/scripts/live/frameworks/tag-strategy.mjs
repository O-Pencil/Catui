 ...
 * `.catpaw/live/config.json`. This module owns that block: building it,
...
export const MARKER_OPEN_TEXT = 'catpaw-live-start';
export const MARKER_CLOSE_TEXT = 'catpaw-live-end';

/** Markers that identify a file as still carrying our tag-strategy patch. */
export const TAG_PATCH_MARKERS = Object.freeze([MARKER_OPEN_TEXT, 'data-catpaw-csp-original']);
...
export function removeTag(content, _syntax) {
  const patterns = [
    /([ \t]*)<!--\s*catpaw-live-start\s*-->[\s\S]*?<!--\s*catpaw-live-end\s*-->([ \t]*(?:\r\n|\n|\r|$)?)/,
    /([ \t]*)\{\/\*\s*catpaw-live-start\s*\*\/\}[\s\S]*?\{\/\*\s*catpaw-live-end\s*\*\/\}([ \t]*(?:\r\n|\n|\r|$)?)/,
  ];
...
// and stash the original `content` value in a `data-catpaw-csp-original`
...
const CSP_MARKER_ATTR = 'data-catpaw-csp-original';
