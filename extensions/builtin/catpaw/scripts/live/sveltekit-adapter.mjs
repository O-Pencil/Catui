...
export const SVELTE_LIVE_ROOT_COMPONENT = 'src/lib/catpaw/CatpawLiveRoot.svelte';
export const SVELTE_LAYOUT_MARKER_OPEN = '<!-- catpaw-live-svelte-start -->';
export const SVELTE_LAYOUT_MARKER_CLOSE = '<!-- catpaw-live-svelte-end -->';
export const SVELTE_ROOT_IMPORT = "import CatpawLiveRoot from '$lib/catpaw/CatpawLiveRoot.svelte';";
...
const SVELTE_ROOT_IMPORT_LINE_RE = /^[ \t]*import CatpawLiveRoot from '\$lib\/catpaw\/CatpawLiveRoot\.svelte(?:\?[^']*)?';[ \t]*\r?\n?/gm;
...
export function svelteRootImportLine(rev) {
  ...
  return "import CatpawLiveRoot from '$lib/catpaw/CatpawLiveRoot.svelte?catpaw-live=" + rev + "';";
}
...
export function patchSvelteLayout(content, { rev = null } = {}) {
  ...
  if (!out.includes(importLine)) {
    ...
    // indentation; only a layout with no catpaw import gets an insert.
    let replaced = false;
...
export function buildSvelteLiveRootComponent(port, token) {
  ...
  const HOST_ID = 'catpaw-live-root';

  onMount(() => {
    let host = document.querySelector('catpaw-live-root#' + HOST_ID) || document.getElementById(HOST_ID);
    if (!host) {
      host = document.createElement('catpaw-live-root');
      host.id = HOST_ID;
      document.body.appendChild(host);
    }

    host.dataset.catpawLiveAdapter = 'sveltekit';
    ...
    if (!root.querySelector('style[data-catpaw-live-reset]')) {
      const reset = document.createElement('style');
      reset.dataset.catpawLiveReset = 'true';
      reset.textContent = ':host, :host *, * { box-sizing: border-box; }';
      root.appendChild(reset);
    }

    window.__CATPAW_LIVE_ADAPTER__ = 'sveltekit';
    window.__CATPAW_LIVE_UI_ROOT__ = root;
    window.__CATPAW_LIVE_CHROME_MOUNT__ = {
      adapter: 'sveltekit',
      version: 1,
      host,
      root,
    ...
    script.src = LIVE_URL;
    script.async = true;
    script.dataset.catpawLiveScript = 'true';
    script.onerror = () => console.error(
      '[catpaw] live.js failed to load from ' + LIVE_URL
    ...
    return () => {
      script.remove();
      if (window.__CATPAW_LIVE_UI_ROOT__ === root) delete window.__CATPAW_LIVE_UI_ROOT__;
      if (window.__CATPAW_LIVE_CHROME_MOUNT__?.root === root) delete window.__CATPAW_LIVE_CHROME_MOUNT__;
      if (window.__CATPAW_LIVE_ADAPTER__ === 'sveltekit') delete window.__CATPAW_LIVE_ADAPTER__;
    };
  });
</script>
`;
