 ...
 *   sessionRoot <appRoot>/.catpaw/live — durable live state.
 *
 ...
 *
 * The resolved manifest is persisted at <appRoot>/.catpaw/live/roots.json
 * plus a pointer at <repoRoot>/.catpaw/live/app-root.json when the two
...
function isAppRoot(dir) {
  ...
  return hasDevConfig(dir) || exists(path.join(dir, '.catpaw', 'live', 'config.json'));
}
...
export function resolveRoots({ cwd = process.cwd(), targetPath = null } = {}) {
  ...
  return {
    manifest: {
      ...
      contextRoot,
      sessionRoot: path.join(appRoot, '.catpaw', 'live'),
      productPath,
      designPath,
      resolvedFrom,
    },
...
function rootsFilePath(appRoot) {
  return path.join(appRoot, '.catpaw', 'live', ROOTS_FILE);
}

function pointerFilePath(repoRoot) {
  return path.join(repoRoot, '.catpaw', 'live', POINTER_FILE);
}
...
function hasLiveServer(appRoot) {
  ...
  let token;
  try {
    const info = JSON.parse(fs.readFileSync(path.join(appRoot, '.catpaw', 'live', 'server.json'), 'utf-8'));
...
function hasActiveDurableSession(appRoot) {
  const dir = path.join(appRoot, '.catpaw', 'live', 'sessions');
  let entries;
  try {
 ...
 *
 *   1. Walk up from cwd looking for .catpaw/live/roots.json.
 *   2. At the git root, follow .catpaw/live/app-root.json to the app.
      ...
      if (candidates.length > 0) {
        ...
        if (tier.length > 1) {
          ...
          process.stderr.write(
            `[catpaw live] Multiple apps in this repo have live state; using ${chosen}. `
...
export function enterLiveRoot(cwd = process.cwd()) {
  ...
  try {
    ...
    console.error(`[catpaw live] ${err.message}`);
    process.exit(1);
  }
  ...
  if (path.resolve(cwd) !== path.resolve(appRoot)) {
    ...
    // to guess.
    if (!isDir(appRoot)) {
      console.error(`[catpaw live] resolved app root does not exist: ${appRoot} (stale roots manifest? re-run the live boot, or pass --target <path>)`);
      process.exit(1);
    }
    try {
      process.chdir(appRoot);
    } catch (err) {
      console.error(`[catpaw live] could not enter app root ${appRoot}: ${err.message}`);
      process.exit(1);
    }
  }
