...
export async function wrapCli() {
  ...
  if (args.includes('--help') || args.includes('-h')) {
    console.log(`Usage: catpaw wrap [options]
...
The agent should insert variant HTML at insertLine.`);
  ...
  const wrapperLines = isJsx ? [
    indent + '<div data-catpaw-variants="' + id + '" data-catpaw-variant-count="' + count + '"' + ' ' + styleContents + '>',
    indent + '  ' + commentSyntax.open + ' catpaw-variants-start ' + id + ' ' + commentSyntax.close,
    indent + '  ' + commentSyntax.open + ' Original ' + commentSyntax.close,
    indent + '  <div data-catpaw-variant="original">',
    reindentOriginal('    '),
    indent + '  </div>',
    indent + '  ' + commentSyntax.open + ' Variants: insert below this line ' + commentSyntax.close,
    indent + '  ' + commentSyntax.open + ' catpaw-variants-end ' + id + ' ' + commentSyntax.close,
    indent + '</div>',
  ] : [
    indent + commentSyntax.open + ' catpaw-variants-start ' + id + ' ' + commentSyntax.close,
    indent + '<div data-catpaw-variants="' + id + '" data-catpaw-variant-count="' + count + '"' + ' ' + styleContents + '>',
...
function buildCssSelectorPrefixExamples(styleMode, count) {
  ...
  return Array.from({ length: count }, (_, i) => `[data-catpaw-variant="${i + 1}"]`);
}
...
function buildCssAuthoring(styleMode, count) {
  ...
  if (styleMode.mode === 'astro-global-prefixed') {
    return {
      ...
      rulePattern: '[data-catpaw-variant="N"] > .variant-class { ... }',
      selectorExamples: variantNumbers.map((n) => `[data-catpaw-variant="${n}"] > .variant-class`),
      requirements: [
        ...
        'Prefix every preview selector with the matching [data-catpaw-variant="N"] selector.',
  ...
  return {
    ...
    styleTag: styleMode.styleTag,
    strategy: 'scope-rule',
    rulePattern: '@scope ([data-catpaw-variant="N"]) { :scope > .variant-class { ... } }',
    selectorExamples: variantNumbers.map((n) => `@scope ([data-catpaw-variant="${n}"]) { :scope > .variant-class { ... } }`),
    requirements: [
      'Use @scope blocks keyed to each [data-catpaw-variant="N"] wrapper.',
    ...
    forbidden: [
      'Do not use global [data-catpaw-variant="N"] selector prefixes for this styleMode.',
 ...
 *
 * Only `node_modules`, `.git`, and `.catpaw` are skipped outright.
...
function findElement(lines, query, tag = null) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (lines[i].includes('data-catpaw-variant')) continue;
