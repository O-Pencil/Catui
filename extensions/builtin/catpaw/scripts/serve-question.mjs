 ...
 * browser is available (CATPAW_QUESTION_DISABLED, or a detected
 * CI/headless/remote environment; CATPAW_QUESTION_FORCE=1 overrides
 ...
 *       "sketch": ".catpaw/sketches/assigned.webp",  // optional; may not exist
...
if (process.env.CATPAW_QUESTION_DISABLED) {
...
if (wantsBrowser && !process.env.CATPAW_QUESTION_FORCE) {
  const headless =
    process.env.CI ||
    (process.env.SSH_CONNECTION && !process.env.DISPLAY) ||
    (process.platform === 'linux' && !process.env.DISPLAY && !process.env.WAYLAND_DISPLAY);
  if (headless) {
    console.log('serve-question: no browser detected in this environment (CI/headless/remote); use the structured question tool instead. Set CATPAW_QUESTION_FORCE=1 to serve anyway.');
    process.exit(2);
  }
}
...
const QUESTION_DIR = path.join(process.cwd(), '.catpaw', 'questions');
...
if (hasFlag('schema')) {
  console.log(JSON.stringify({
    ...
    question: 'The roll assigned Fillmore Handbill. Keep it, take an alternate, or re-roll.',
    options: [
      { id: 'assigned', label: 'Fillmore Handbill', kicker: 'THE ROLL', lineage: '1966-71 Fillmore psychedelic handbills', thesis: 'The gig poster that treats every release like a one-night stand.', palette: ['#e8452c', '#f5d64c', '#1b2a52', '#f3ead8'], materials: ['letterpress', 'split-fountain ink'], viewport: 'A full-bleed dated bill with the product name in warped display type.', risk: 'Reads nostalgic when the type is set timidly.', sketch: '.catpaw/sketches/assigned.webp', hero: 'https://catpaw.style/worlds/cards/fillmore-handbill-hero.webp', board: 'https://catpaw.style/worlds/cards/fillmore-handbill.webp' },
      { id: 'challenger-teletext', label: 'Teletext Service', lineage: 'broadcast teletext magazines', thesis: 'The catalog as a broadcast index: pages, not sections.', case: 'Fuses cleanly: releases map to numbered pages.', sketch: '.catpaw/sketches/challenger-teletext.webp', hero: 'https://catpaw.style/worlds/cards/broadcast-programming-teletext-service-hero.webp' },
    ],
    reroll: true,
    canon: true,
    canonCard: { label: 'The category standard', thesis: 'What this category ships, executed impeccably.', viewport: 'The arrangement a visitor expects, at full craft.', sketch: '.catpaw/sketches/canon.webp' },
    steer: true,
  }, null, 2));
...
<title>${esc(payload.title || 'catpaw · decision')}</title>
...
<style>
  /* Neo kinpaku tokens, mirrored from catpaw.style kinpaku-tokens.css */
  :root {
    color-scheme: dark;
...
<header>
  <div class="brand">
    ...
    <span class="wordmark">Catpaw</span>
  </div>
</header>
<main>
  <div class="stage">
