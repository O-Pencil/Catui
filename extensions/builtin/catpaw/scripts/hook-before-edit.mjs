#!/usr/bin/env node
/**
 * Catpaw design hook — Cursor preToolUse write gate.
 *
...
async function detectProposedHtml(detector, content, filePath, scanOptions) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'catpaw-pre-'));
...
function cursorBlockMessage(findings, filePath, config, cwd) {
  ...
  const blocked = rendered.replace(
    '[catpaw@1] Design hook findings requiring review',
    '[catpaw@1] Catpaw design hook blocked this write before it landed. Design hook findings requiring review',
  );
...
async function main() {
  if (truthy(process.env.CATPAW_HOOK_DISABLED)) {
  ...
  if (denial.count > EDIT_COUNT_THRESHOLD) {
    const warning = `${message}\n\nThis is the ${denial.count}th repeated denial for the same file and finding signature, so Catpaw is allowing this write to avoid a loop. Reconsider the issue immediately after the tool runs.`;
    return allow({
      ...audit,
      findings: (findings || []).length,
...
main().catch((err) => {
  if (process.env.CATPAW_HOOK_DEBUG) {
    process.stderr.write(`[catpaw-hook-before-edit] ${err}\n`);
  }
