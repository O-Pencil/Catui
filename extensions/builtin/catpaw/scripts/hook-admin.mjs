#!/usr/bin/env node
/**
 * The Catpaw hooks command manages the design hook runtime
 * via the `hook` key and shared detector ignores via the `detector` key in
 * .catpaw/config.json / .catpaw/config.local.json.
 *
 * Usage:
...
import { CATPAW_COMMAND } from './lib/provider.mjs';
...
const CATPAW_HOOK_COMMAND_MARKERS = [
  'skills/catpaw/scripts/hook-probe.mjs',
  'skills/catpaw/scripts/hook.mjs',
  'skills/catpaw/scripts/hook-before-edit.mjs',
  'skills/catpaw/scripts/hook-after-edit.mjs',
  'skills/catpaw/scripts/hook-stop.mjs',
];
...
const HOOK_MANIFEST_TARGETS = [
  {
    provider: '.claude',
    skillRel: '.claude/skills/catpaw',
    destRel: '.claude/settings.local.json',
    sharedDestRel: '.claude/settings.json',
    manifest: () => ({
      description: 'Catpaw design detector: immediate-tier checks after Edit/Write/MultiEdit on UI files, full-rule deep pass on Stop.',
      hooks: {
        PostToolUse: [
          {
            ...
            hooks: [
              {
                type: 'command',
                command: 'node "${CLAUDE_PROJECT_DIR}/.claude/skills/catpaw/scripts/hook.mjs"',
        ...
        Stop: [stopManifestEntry('node "${CLAUDE_PROJECT_DIR}/.claude/skills/catpaw/scripts/hook.mjs"')],
      },
    }),
  },
  {
    provider: '.agents',
    skillRel: '.agents/skills/catpaw',
    ...
    manifest: () => ({
      hooks: {
        PostToolUse: [
          {
            ...
            hooks: [
              {
                type: 'command',
                command: 'node ".agents/skills/catpaw/scripts/hook.mjs"',
                timeout: TIMEOUT_SECONDS,
                statusMessage: STATUS_MESSAGE,
              },
            ],
        ...
        Stop: [stopManifestEntry('node ".agents/skills/catpaw/scripts/hook.mjs"')],
      },
    }),
  },
  {
    provider: '.cursor',
    skillRel: '.cursor/skills/catpaw',
    ...
    manifest: () => ({
      ...
      hooks: {
        preToolUse: [
          {
            command: 'node ".cursor/skills/catpaw/scripts/hook-before-edit.mjs"',
            timeout: TIMEOUT_SECONDS,
          },
        ],
      },
  ...
  },
  {
    ...
    provider: '.github',
    skillRel: '.github/skills/catpaw',
    destRel: '.github/hooks/catpaw.json',
    manifest: () => ({
      version: 1,
      hooks: {
        postToolUse: [
          {
            ...
            bash: 'node "$(git rev-parse --show-toplevel)/.github/skills/catpaw/scripts/hook.mjs"',
...
function writeHookConfig(cwd, hookConfig, opts = {}) {
  ...
  // (consent, quiet, auditLog) survive an Catpaw hooks edit.
...
function statusReport(cwd) {
  ...
  const envKill = process.env.CATPAW_HOOK_DISABLED;
  const envState = envKill ? `CATPAW_HOOK_DISABLED=${envKill}` : 'unset';
  const cfgPath = path.relative(cwd, getConfigPath(cwd)) || '.catpaw/config.json';
  const localPath = path.relative(cwd, getLocalConfigPath(cwd)) || '.catpaw/config.local.json';
  const cachePath = path.relative(cwd, getCachePath(cwd)) || '.catpaw/hook.cache.json';
  ...
  // `rule=value [files]` shape `catpaw ignores list` already prints.
...
function repairHookManifests(cwd) {
  ...
  for (const target of HOOK_MANIFEST_TARGETS) {
    ...
    if (sharedDest && fileHasCatpawHookMarker(sharedDest)) {
      pruneCatpawHookFromManifest(dest);
...
function mergeHookManifests(existing, fresh) {
  ...
  for (const event of hookEvents) {
    const preserved = stripCatpawHookEntries(existingHooks[event]);
...
function fileHasCatpawHookMarker(filePath) {
  ...
  return valueHasCatpawHookMarker(parsed.hooks);
}

function valueHasCatpawHookMarker(value) {
  if (typeof value === 'string') {
    return CATPAW_HOOK_COMMAND_MARKERS.some((marker) => value.includes(marker));
  }
  if (Array.isArray(value)) return value.some(valueHasCatpawHookMarker);
  if (value && typeof value === 'object') return Object.values(value).some(valueHasCatpawHookMarker);
  return false;
}

function stripCatpawHookEntry(entry) {
  ...
  if (valueHasCatpawHookMarker(entry.command) || valueHasCatpawHookMarker(entry.args)
    || valueHasCatpawHookMarker(entry.bash) || valueHasCatpawHookMarker(entry.powershell)) {
    return null;
  }
  if (!Array.isArray(entry.hooks)) return entry;

  const strippedHooks = entry.hooks
    .map(stripCatpawHookEntry)
    .filter(Boolean);

  if (strippedHooks.length === 0 && entry.hooks.some(valueHasCatpawHookMarker)) {
    return null;
  }
  return { ...entry, hooks: strippedHooks };
}

function stripCatpawHookEntries(entries) {
...
function pruneCatpawHookFromManifest(manifestPath) {
  if (!fileHasCatpawHookMarker(manifestPath)) return false;
  let parsed;
  try {
  ...
  for (const [event, entries] of Object.entries(existingHooks)) {
    const kept = stripCatpawHookEntries(entries);
...
function addIgnoreRule(cwd, args) {
  ...
  if (!rule) throw new Error(`Pass a rule id, e.g. ${CATPAW_COMMAND} hooks ignore-rule side-tab`);
  if (rule === 'overused-font' && !parsed.allValues) {
    throw new Error(`overused-font is value-specific by default. Use ${CATPAW_COMMAND} hooks ignore-value overused-font <font> for a confirmed font, or ${CATPAW_COMMAND} hooks ignore-rule overused-font --all-values only when the user asked to ignore overused fonts generally.`);
  }
...
function addIgnoreFile(cwd, args) {
  ...
  if (!glob) throw new Error(`Pass a glob, e.g. ${CATPAW_COMMAND} hooks ignore-file "src/legacy/**"`);
...
function parseIgnoreValueArgs(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === '--shared') {
      ...
      // reported success. Matches `catpaw ignores add-value`.
...
function addIgnoreValue(cwd, args) {
  ...
  if (!parsed.rule || !parsed.value) {
    throw new Error(`Pass a rule id and value, e.g. ${CATPAW_COMMAND} hooks ignore-value overused-font Inter`);
  }
  ...
  // `files` scope, so require one — matching `catpaw ignores add-value`.
  if (parsed.value === '*' && parsed.files.length === 0) {
    ...
    const projectWide = parsed.rule === 'overused-font'
      ? `${CATPAW_COMMAND} hooks ignore-rule ${parsed.rule} --all-values`
      : `${CATPAW_COMMAND} hooks ignore-rule ${parsed.rule}`;
    throw new Error(`Wildcard value ignores must be scoped with --file <glob>, e.g. ${CATPAW_COMMAND} hooks ignore-value design-system-font-size "*" --file "src/widget.js". To suppress the rule project-wide use ${projectWide}.`);
  }
