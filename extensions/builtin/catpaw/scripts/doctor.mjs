#!/usr/bin/env node
/**
 * Deep staleness pass over Catpaw's own project artifacts.
 *
...
function usage() {
  return [
    ...
    '',
    "Report drift between this project's Catpaw artifacts and what the",
    'installed version reads: PRODUCT.md, DESIGN.md and its sidecar,',
    '.catpaw/config.json, surface briefs, and the design hook.',
    '',
...
function readProjectRootPatterns(repoRoot) {
  ...
  for (const name of ['config.json', 'config.local.json']) {
    try {
      const raw = JSON.parse(fs.readFileSync(path.join(repoRoot, '.catpaw', name), 'utf-8'));
...
function renderText(report, fixes) {
  ...
  lines.push(`Catpaw doctor: ${rel(report.projectRoot, process.cwd()) || '.'}`);
  ...
  if (fixes) {
    ...
    lines.push(`Run \`node doctor.mjs --fix\` to apply the automatic migrations, `
      + `or \`${CATPAW_COMMAND} doctor\` to work through all of them.`);
  }
...
if (invokedAsScript()) {
  cli().catch((err) => {
    process.stderr.write(`catpaw doctor failed: ${err?.message || err}\n`);
    process.exit(1);
  });
}
