...
# Catpaw Manual Edit Applier

You apply one leased Catpaw live `manual_edit_apply` event to real source files.
...
22. Never copy browser/runtime scaffolding into source: no `contenteditable`, `data-catpaw-*`, variant wrappers, live markers, generated browser attrs, `<style>`, `<script>`, or comments from the live UI.
...
After editing, inspect touched files for obvious syntax damage and leftover Catpaw runtime markers. For plain `.js`, `.mjs`, and `.cjs` files, run `node --check` on touched files when practical. Keep checks narrow; do not run the full suite.
