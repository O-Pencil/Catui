/**  * [WHO]: ArenaRunner - spawns parallel subagents racing the same problem  * [FROM]: Depends on core/sub-agent, core/workspace, core/tools  * [TO]: Consumed by arena extension index.ts  * [HERE]: extensions/builtin/arena/arena-runner.ts - Arena orchestration logic  */ import type { Model } from "@catui/ai/types";
import {
  SubAgentRuntime,
  InProcessSubAgentBackend,
} from "../../../core/sub-agent/index.js";
...
export class ArenaRunner {
  ...
  constructor() {
    this.runtime = new SubAgentRuntime(
      new InProcessSubAgentBackend(createAgentSession),
    );
    this.worktreeManager = new WorktreeManager();
  }
  isRunning(): boolean {
    return (
      this.currentReport !== null && this.currentReport.endTime === undefined
    );
  }
  ...
  async run(options: ArenaRunOptions): Promise<ArenaReport> {
    if (this.isRunning()) {
      throw new Error(
        "An Arena run is already in progress. Stop it before starting a new one.",
      );
    }
    ...
    const contestants: ArenaContestant[] = strategies.map(
      (strategy, index) => ({
    ...
    try {
      const worktrees = await Promise.all(
        strategies.map(() =>
          this.worktreeManager.createGitWorktree(undefined, options.cwd),
        ),
      );
      const tools = strategies.map((_, index) =>
        createCodingTools(worktrees[index]!.path),
      );
      const spawnPromises = contestants.map(async (contestant, index) => {
        contestant.worktree = worktrees[index]!;
        const prompt = this.buildContestantPrompt(
          options.problem,
          contestant.strategy,
          index + 1,
          strategies.length,
  ...
  private buildContestantPrompt(
    problem: string,
    strategy: string,
    index: number,
    total: number,
  ): string {
    return `You are contestant ${index} of ${total} in an Arena challenge.  ## Problem to solve: ${problem}  ## Your assigned strategy/approach: ${strategy}  ## Instructions: - Solve the problem using ONLY the strategy described above - Work in your isolated workspace (you have your own git worktree) - Be thorough but efficient - When done, summarize what you accomplished and any trade-offs  Begin working on the problem now.`;
  }
  private selectWinner(): void {
    if (!this.currentReport) return;
    const completed = this.currentReport.contestants.filter(
