/**  * [WHO]: Arena extension - parallel subagent racing for the same problem  * [FROM]: Depends on @catui/tui, core/extensions-host/types, ./arena-runner, ./arena-types  * [TO]: Consumed by builtin-extensions.ts as default extension  * [HERE]: extensions/builtin/arena/index.ts - entry point for Arena mode  */ import {
  Box,
  Container,
  Spacer,
  Text,
...
const ARENA_ROOT_COMPLETIONS = [
  {
    value: "run",
    label: "run",
    description: "Start an Arena race with strategies",
  },
  { value: "stop", label: "stop", description: "Stop the current Arena race" },
  {
    value: "status",
    label: "status",
    description: "Show current Arena status",
  },
  {
    value: "results",
    label: "results",
    description: "Show latest Arena results",
  },
  {
    value: "cleanup",
    label: "cleanup",
    description: "Clean up Arena worktrees",
  },
...
function formatReport(report: ArenaReport): string {
  ...
  lines.push("");
  lines.push(
    `**Problem:** ${report.problem.slice(0, 200)}${report.problem.length > 200 ? "..." : ""}`,
  );
  lines.push("");
  const totalDuration = report.endTime
    ? report.endTime - report.startTime
    : Date.now() - report.startTime;
      ...
      contestant.status === "completed"
        ...
        : contestant.status === "failed"
          ...
          : contestant.status === "aborted"
            ? "⊘"
            : "⟳";
    const duration = contestant.endTime
      ? formatDuration(contestant.endTime - contestant.startTime)
      : "running...";
    const winnerTag = isWinner ? " **(winner)**" : "";
    lines.push(
      `- ${statusIcon} **${contestant.id}**${winnerTag} [${duration}]`,
    );
    lines.push(
      `  Strategy: ${contestant.strategy.slice(0, 100)}${contestant.strategy.length > 100 ? "..." : ""}`,
    );
    if (contestant.result?.response) {
      const responsePreview = contestant.result.response.slice(0, 300);
      lines.push(
        `  Result: ${responsePreview}${contestant.result.response.length > 300 ? "..." : ""}`,
      );
    }
    if (contestant.error) {
  ...
  if (report.winnerId) {
    lines.push(
      `**Winner:** ${report.winnerId} (fastest successful completion)`,
    );
  } else {
...
function buildArenaHelp(): string {
  return `## Arena Mode - Parallel Problem Solving  Arena spawns multiple subagents racing to solve the same problem, each with a different strategy/approach. Results are compared and the fastest successful solution wins.  ### Commands: - \`/arena:run <problem> --strategies "strategy1|strategy2|..."\` - Start a race - \`/arena:stop\` - Stop the current race - \`/arena:status\` - Show current race status - \`/arena:results\` - Show latest results - \`/arena:cleanup\` - Clean up worktrees from last race - \`/arena:help\` - Show this help  ### Example: \`\`\` /arena:run Implement a binary search tree --strategies "recursive approach|iterative with stack|functional style" \`\`\`  ### Notes: - Maximum 4 contestants per race - Each contestant gets an isolated git worktree - Winner is the fastest successful completion - Use \`/arena:cleanup\` to remove worktrees after reviewing results `;
}
export default async function arenaExtension(api: ExtensionAPI): Promise<void> {
  ...
  for (const commandName of commandNames) {
    api.registerCommand(commandName, {
      ...
      getArgumentCompletions: (argumentPrefix, _context) =>
        getArgumentCompletions(commandName, argumentPrefix),
      handler: async (args: string, ctx) => {
        ...
        switch (commandName) {
          ...
          case "arena": {
            api.sendMessage({
              customType: ARENA_MESSAGE_TYPE,
              content: buildArenaHelp(),
              display: true,
            });
            break;
          }
          case "arena:run": {
            if (arenaRunner.isRunning()) {
              ctx.ui.notify(
                "An Arena race is already in progress. Use /arena:stop to cancel it.",
                "error",
              );
              return;
            }
            ...
            if (!parsed) {
              ctx.ui.notify(
                'Usage: /arena:run <problem> --strategies "strategy1|strategy2|..."',
                "error",
              );
              return;
            }
            ctx.ui.notify(
              `Arena: starting race with ${parsed.strategies.length} contestants...`,
              "info",
            );
            try {
              ...
              ctx.ui.notify(
                `Arena failed: ${error instanceof Error ? error.message : String(error)}`,
                "error",
              );
            }
            break;
          ...
          case "arena:status": {
            ...
            if (arenaRunner.isRunning()) {
              const running = report.contestants.filter(
                (c) => c.status === "running",
              ).length;
              const completed = report.contestants.filter(
                (c) => c.status === "completed",
              ).length;
              const failed = report.contestants.filter(
                (c) => c.status === "failed",
              ).length;
              ctx.ui.notify(
                `Arena ${report.arenaId} in progress: ${running} running, ${completed} completed, ${failed} failed`,
                "info",
              );
            } else {
              api.sendMessage({
      ...
      return [
        {
          value: '--strategies "',
          label: "--strategies",
          description: "Pipe-separated strategies",
        },
      ];
    }
    return null;
  }
  const matches = ARENA_ROOT_COMPLETIONS.filter((completion) =>
...
function parseArenaRun(
  args: string,
): { problem: string; strategies: string[] } | null {
