...
export function sandboxCommand(command: string, args: string[], cwd: string, temp: string, platform = process.platform): { command: string; args: string[] } {
	if (platform === "darwin") {
	...
	if (platform === "linux") {
		...
		return { command: "bwrap", args: ["--die-with-parent", "--unshare-net", "--unshare-pid", "--ro-bind", "/", "/", ...binds, "--bind", cwd, cwd, "--bind", temp, temp, "--ro-bind", join(cwd, ".git"), join(cwd, ".git"), "--dev", "/dev", "--proc", "/proc", "--chdir", cwd, "--", command, ...args] };
	}
	throw new Error("Autonomous verification requires macOS sandbox-exec or Linux bubblewrap");
}
export function verificationRunner(run: RunCommand): RunCommand {
	return async (command, args, options) => {
		const temp = join(tmpdir(), "catui-evolution-verify", String(process.pid));
		await mkdir(temp, { recursive: true, mode: 0o700 });
		const confined = sandboxCommand(command, args, await realpath(options.cwd), await realpath(temp));
