...
export async function runWorker(config: SourceConfig, phase: string, testPath: string, prompt: string): Promise<string> {
	if (!["triage", "reproduce", "repair", "review"].includes(phase)) throw new Error("Invalid worker phase");
	...
	const model = slash > 0 ? registry.find(config.model.slice(0, slash), config.model.slice(slash + 1)) : registry.getAll().find(m => m.id === config.model);
	if (!model) throw new Error(`Configured evolution model is unavailable: ${config.model}`);
	const readonly = phase === "review" || phase === "triage";
	...
	const guarded = tools.map(tool => ({ ...tool, execute: async (id, params, signal, update) => {
		...
		if (tool.name === "write" || tool.name === "edit") {
			if (phase === "reproduce" ? rel !== testPath : !repairable(rel)) throw new Error("Worker cannot alter acceptance tests, delivery policy, dependencies or release configuration");
		}
