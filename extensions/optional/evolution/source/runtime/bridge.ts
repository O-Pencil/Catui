...
import { isCorrection, taskCohort } from "../learning/detectors.js";
...
export function registerSourceEvolution(api: ExtensionAPI): void {
	...
	const emit = (event: Omit<Observation, "id" | "time" | "run" | "fingerprint">) => {
		...
		pending++;
		void spool(root, { ...event, ...cohort, id: randomUUID(), time: new Date().toISOString(), run, fingerprint: fingerprint(`${event.version}:${event.tool ?? event.kind}`, event.summary) })
			.catch(() => {
	...
	api.on("before_agent_start", (event, ctx) => {
		if (!root) return;
		run = randomUUID(); starts.clear(); intents.clear(); inputs.clear(); repeated.clear();
		cohort = taskCohort(event.prompt); answer = "";
		const correction = isCorrection(event.prompt);
	...
	api.on("tool_execution_start", event => {
		...
		intents.set(event.toolCallId, sanitize(typeof event.args.command === "string" ? event.args.command : event.toolName).slice(0, 400));
	...
	api.on("tool_execution_end", (event, ctx) => {
		...
		const text = event.isError ? (result?.content ?? []).filter(p => p.type === "text").map(p => p.text ?? "").join("\n") : repeated.has(event.toolCallId) ? "Repeated identical tool request" : durationMs > 120000 ? "Tool took over two minutes" : `Tool completed: ${intents.get(event.toolCallId) ?? event.toolName}`;
		...
		intents.delete(event.toolCallId);
		repeated.delete(event.toolCallId);
	});
	api.on("turn_end", (event, ctx) => {
		if (!root) return;
		if (event.message.role !== "assistant") return;
		answer = sanitize(event.message.content.filter(p => p.type === "text").map(p => p.text).join("\n"));
	...
	api.on("agent_result", (event, ctx) => {
		if (!root) return;
		emit({ kind: "result", session: ctx.sessionManager.getSessionId(), workspace: digest(ctx.cwd), version: VERSION, model: ctx.model?.id ?? "unknown", failed: Boolean(event.errorMessage), summary: sanitize(event.errorMessage ?? (answer || event.stopReason || "Completed")), durationMs: event.durationMs, toolCalls: event.toolCallCount, turns: event.turnCount });
	});
	api.on("session_shutdown", () => { root = undefined; starts.clear(); intents.clear(); inputs.clear(); repeated.clear(); answer = ""; });
}
