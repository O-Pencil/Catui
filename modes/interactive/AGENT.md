...
controllers/extension-ui/prompt-host.ts: PromptHost, PromptHostContext — single-active-prompt slot for selector/confirm/input/editor dialogs; held as `this.promptHost`, wired into ExtensionUIContext select/confirm/input/editor + focus restore; also serves self-update and auth/provider-config prompts (option picker + input) through the same slot; P5 extension-ui host 2/4 (UI02, 重写)
...
controllers/auth-provider-config-controller.ts: AuthProviderConfigController, AuthProviderConfigContext — interactive API key/OAuth/custom-provider configuration; owns /apikey, /login, /logout and provider precondition flow consumed by model-overlay; built-in provider selection shows a Browse models / API-key action menu (credential updates stay reachable, parity with custom protocol providers); no InteractiveMode reference (P5 UI02/UI03, hybrid)
...
controllers/session-command-handlers.ts: SessionCommandHandlersController, SessionCommandHandlersContext — session-mutating command bodies (/reload, /clear, /compact, !bash, extension-command check); narrow context of host capability closures, no InteractiveMode reference; extracted from InteractiveMode command-handler block (P7 C-3a slice 1/4, 纯搬)
controllers/persona-command-handlers.ts: PersonaCommandHandlersController, PersonaCommandHandlersContext — persona/soul/memory command bodies (/persona, /soul, /memory, /browser opt-in, applyPersonaFromSessionIfAny); no InteractiveMode reference (P7 C-3a slice 2/4, 纯搬)
controllers/config-command-handlers.ts: ConfigCommandHandlersController, ConfigCommandHandlersContext — runtime-config command bodies (/agent-loop, /mcp, /language); no InteractiveMode reference (P7 C-3a slice 3/4, 纯搬)
controllers/info-command-handlers.ts: InfoCommandHandlersController, InfoCommandHandlersContext — display/info command bodies (/export, /share, /copy, /status, /usage, /name, /session, /changelog, /hotkeys, /resources, easter eggs); no InteractiveMode reference (P7 C-3a slice 4/4, 纯搬)
controllers/chat-renderer.ts: ChatRendererController, ChatRendererContext — transcript rendering: message→component mapping (addMessageToChat), session-context replay (renderSessionContext), initial transcript + welcome banner (renderInitialMessages), chat rebuild with optimistic-user dedupe (rebuildChatFromMessages); held lazily as `this.chatRenderer` behind thin delegators so partial-mode tests keep monkey-patching host methods; internal cross-calls route through ctx callbacks (P7 C-3c, 纯搬)
controllers/external-editor-controller.ts: ExternalEditorController, ExternalEditorContext — $VISUAL/$EDITOR round-trips (openExternalEditor temp-file flow, openExistingFileInExternalEditor); held lazily as `this.externalEditor` behind thin delegators; no InteractiveMode reference (P7 C-3d, 纯搬)
controllers/buddy-pet-controller.ts: BuddyPetController, BuddyPetContext — desktop-pet lifecycle (syncBuddyPet enable/species handling, setBuddyPetState with reset timer, dispose); pet/species/timer state owned privately here; held lazily as `this.buddyPetHost` (P7 C-3d, 纯搬)
controllers/agent-run-timer-controller.ts: AgentRunTimerController, AgentRunTimerContext — working-message rotation (cat messages), elapsed formatting, agent-run interval timer; cat-message fields owned privately here; held lazily as `this.agentRunTimer` (P7 C-3d, 纯搬)
controllers/status-display-controller.ts: StatusDisplayController, StatusDisplayContext — chat status/error/warning lines with auto-dismiss timers and dedup-by-last-line, priority notifications; statusTimers owned privately here; held lazily as `this.statusDisplay` behind thin delegators so partial-mode tests keep monkey-patching host methods (P7 C-3d, 纯搬)
controllers/message-queue-controller.ts: MessageQueueController, MessageQueueContext — steering/follow-up/compaction queue management (handleFollowUp, handleDequeue, promptAfterRender, getAllQueuedMessages, clearAllQueues, updatePendingMessagesDisplay, restoreQueuedMessagesToEditor, queueCompactionMessage, flushCompactionQueue); internal cross-calls are direct same-controller calls, host services (showStatus/promptAfterRender entry) route through ctx callbacks; held lazily as `this.messageQueue` (P7 C-3d, 纯搬)
