...
export const builtInExtensions: readonly BuiltinExtension[] = [
	...
	{ id: "grub", category: "default", defaultEnabled: true, riskLevel: "background", requiresUI: false, startsTimers: false, writesWorkspace: false, externalProcess: true, testContracts: ["lifecycle", "external-process"], testFiles: ["test/grub-controller.test.ts", "test/goal-grub-lifecycle.test.ts"] },
	{ id: "goal", category: "default", defaultEnabled: true, riskLevel: "background", requiresUI: false, startsTimers: false, writesWorkspace: false, externalProcess: false, testContracts: ["lifecycle"], testFiles: ["test/goal-controller.test.ts", "test/goal-grub-lifecycle.test.ts"] },
	...
	{ id: "humanizer", category: "default", defaultEnabled: true, riskLevel: "passive", requiresUI: false, startsTimers: false, writesWorkspace: false, externalProcess: false, resourceDiscovery: true, testContracts: ["resource-discovery"], testFiles: ["test/humanizer-extension.test.ts"] },
...
export function getBuiltinExtensionPaths(): string[] {
	...
	// === Humanizer extension (vendored writing-quality skill) ===
	if (existsSync(BUNDLED_HUMANIZER_EXTENSION)) {
		paths.push(BUNDLED_HUMANIZER_EXTENSION);
	} else {
		const humanizerTs = join(__dirname, "extensions", "builtin", "humanizer", "index.ts");
		if (existsSync(humanizerTs)) paths.push(humanizerTs);
	}
