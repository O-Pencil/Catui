...
runner.ts: ExtensionRunner class, extension execution and lifecycle management, event emission to hooks, tool wrapping, key hooks: session_start, session_shutdown, before_agent_start, agent_result, after_agent_end, tool_call; owns invokeCommand() — single chokepoint for slash command dispatch with built-in telemetry middleware writing to ext_command_events via core/platform/telemetry/ExtensionTelemetrySink
