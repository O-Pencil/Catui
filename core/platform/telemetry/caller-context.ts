 ...
 * [TO]: runWithExtCallerContext is pushed by core/extensions-host/runner.ts at every command/hook dispatch boundary; getExtCallerContext is read by core/runtime/extension-core-bindings.ts when writing ext_llm_calls rows
 * [HERE]: core/platform/telemetry/caller-context.ts - the only mechanism distinguishing "extension command path (user-initiated)" from "extension hook path (auto-fired)" when an LLM call eventually happens; if this bus is empty, the telemetry layer marks the call as is_user_initiated=false / extension_name="unknown"
 */
