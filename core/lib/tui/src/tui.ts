...
function shouldUseSynchronizedOutput(): boolean {
	if (process.env.CATUI_SYNC_OUTPUT === "0") {
		return false;
	}
	if (process.env.CATUI_SYNC_OUTPUT === "1") {
		return true;
	}
...
export class TUI extends Container {
	...
	private debugLog(msg: string): void {
		if (process.env.CATUI_DEBUG_RENDER !== "1") return;
		const logPath = path.join(os.homedir(), ".catui", "agent", "catui-debug.log");
	...
	private doRender(): void {
		...
		const debugRedraw = process.env.CATUI_DEBUG_REDRAW === "1";
		...
		// Configurable via setClearOnShrink() or CATUI_CLEAR_ON_SHRINK=0 env var
		...
		if (process.env.CATUI_TUI_DEBUG === "1") {
