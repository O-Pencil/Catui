...
describe("CombinedAutocompleteProvider", () => {
	...
	describe("fd @ file suggestions", { skip: !isFdInstalled }, () => {
		...
		test("matches deeply nested paths", () => {
			setupFolder(baseDir, {
				files: {
					"core/lib/tui/src/autocomplete.ts": "export {};",
					"core/lib/ai/src/autocomplete.ts": "export {};",
				},
			});
			...
			const values = result?.items.map((item) => item.value);
			assert.ok(values?.includes("@core/lib/tui/src/autocomplete.ts"));
			assert.ok(!values?.includes("@core/lib/ai/src/autocomplete.ts"));
		});
